...
### 2026-09-15 — A workflow that lists source files breaks on a new import

**What happened.** `Backend Staging CD` failed 28 times in a row across two days. Every run stopped at `Seed staging Admin account` with `error: Cannot find module '@/config/default-database-url' from '/app/seed/src/database/client.ts'`. `bun run check` and `Backend CI` stayed green the whole time, because the missing file exists in the repository; only the deploy step could not see it.
**Root cause.** The step ships a `tar` of hand-listed paths to the staging server and mounts it as `/app/seed`. That list is the transitive import closure of `scripts/seed-admin.ts`, written out by hand. Commit `876d988` added an import of `@/config/default-database-url` to `src/database/client.ts`, which put a 19th file in the closure and left the list at 18. Nothing in CI reads that list, so the drift stays invisible until a deploy runs.
**Rule.** When you add an import to `src/database/client.ts`, `src/config/env.ts`, or any `src/modules/auth/*auth*.ts`, check the file list in the `Seed staging Admin account` step of `.github/workflows/staging-cd.yml`. Derive the closure mechanically — never retype it. A green `Backend CI` is not evidence that the deploy still resolves; `Backend Staging CD` runs only after a push to `develop`, and `workflow_run` takes the workflow file from `main`, so a workflow fix merged to `main` proves nothing until the next push to `develop`.
