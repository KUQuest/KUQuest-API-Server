...
| **Trust & Safety (Messages)**    | Moderation of reported Work Chat and Candidate Inquiry Messages. Decisions: `REPORT_CASE_DISMISSED`, `REPORT_CASE_HIDDEN` (creates Misconduct strike), `REPORT_CASE_RESTORED` (reverses strike). Evidence access via Evidence Reference. | Shared Admin Action infrastructure and the four Trust & Safety persistence tables now exist in `admin.schema.ts`; Report Cases have a generated public sequence for `RPT-######` display IDs; Member intake, Admin moderation, and case-scoped Evidence routes remain absent. | **Partial — persistence foundation exists; routes remain absent**. |
...
| Moderation tables                   | Target requires Report Case, Reporter Entry, Evidence Reference, and Moderation Decision tables. The shared Admin Action table is tracked separately below.                                                 | `admin_report_cases`, `admin_reporter_entries`, `admin_evidence_references`, and `admin_moderation_decisions` exist in `admin.schema.ts` with PostgreSQL constraints and indexes.                                               | **Partial** — route and service behavior remains.                   |
