...
| Moderation tables                   | Target requires Report Case, Reporter Entry, Evidence Reference, and Moderation Decision tables. The shared Admin Action table is tracked separately below.                                                 | `admin_report_cases`, `admin_reporter_entries`, `admin_evidence_references`, and `admin_moderation_decisions` exist in `admin.schema.ts` with PostgreSQL constraints and indexes. Student intake creates the Report Case and Message Evidence Reference; `admin-report.route.ts` and `admin-report.service.ts` provide the Report Case Admin queue, evidence, and decision behavior. | **Partial** — strike, notification, and retention integrations remain. |
| Conduct Report table                | Target requires Conduct Report table keyed to Quest and reported Member with fixed reason enums and Admin decisions.                                                                                        | `adminConductReport` in `admin.schema.ts` and its migration provide the persistence model. Routes and penalty integration remain absent.                                                                                                                                                                                                                                             | **Partial — schema only**.                                             |
