...
| **Trust & Safety (Messages)**    | Moderation of reported Work Chat and Candidate Inquiry Messages. Decisions: `REPORT_CASE_DISMISSED`, `REPORT_CASE_HIDDEN` (creates Misconduct strike), `REPORT_CASE_RESTORED` (reverses strike). Evidence access via Evidence Reference. | Student intake is implemented by `message-report.route.ts` and `message-report.service.ts`; it checks Message visibility, creates Reporter Entries and Message Evidence References, and exposes own status only. Admin moderation and case-scoped Evidence routes remain absent. | **Partial — Student intake implemented; Admin operations remain**. |
...
| Moderation tables                   | Target requires Report Case, Reporter Entry, Evidence Reference, and Moderation Decision tables. The shared Admin Action table is tracked separately below.                                                 | `admin_report_cases`, `admin_reporter_entries`, `admin_evidence_references`, and `admin_moderation_decisions` exist in `admin.schema.ts` with PostgreSQL constraints and indexes. Student intake creates the Report Case and Message Evidence Reference; Admin moderation remains absent. | **Partial** — Admin route and decision behavior remains.            |
