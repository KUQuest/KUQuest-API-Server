...
| -------------------------------- | ---------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | -------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | --------------------------------------------------------------------------------------------------- |
| **Payout Approval**              | Manual Admin approval queue (`PENDING_ADMIN_APPROVAL`), approve/cancel actions with `Idempotency-Key` and current Payout version, masked destination display, Payout worker hand-off.                                                    | `payout.admin.route.ts`, `payout.admin.controller.ts`, `payout.admin.service.ts`; ADR `0022-manual-admin-approval-for-payouts.md`                                                                                                                                                                                                                                                                                                                                                | **Confirmed**.                                                                                      |
...
| **Wallet Freeze/Suspend**        | Admin sets Student Wallet to `FROZEN` or `SUSPENDED` with reason and `Idempotency-Key`. Blocks new financial and Quest commitments while honoring existing obligations. Auto-freeze on ban.                                              | `changeWalletStatus` exists in `wallet.status.service.ts`, but no Admin route calls it. Member Penalty writes automatic ban freezes in its transaction, and an expiry worker restores only automatic freezes.                                                                                                                                                                                                                                                                    | **Partial — ban automation implemented; Admin status route remains**.                               |
| **Trust & Safety (Messages)**    | Moderation of reported Work Chat and Candidate Inquiry Messages. Decisions: `REPORT_CASE_DISMISSED`, `REPORT_CASE_HIDDEN` (creates Misconduct strike), `REPORT_CASE_RESTORED` (reverses strike). Evidence access via Evidence Reference. | Student intake is implemented by `message-report.route.ts` and `message-report.service.ts`. Admin queue, case detail, case-scoped Evidence access, Report Case decisions, strike creation, and reversal are implemented by `admin-report.route.ts` and `admin-report.service.ts`. Push notification and retention-worker integrations remain unavailable.                                                                                                                        | **Partial — Report Case Admin operations implemented; integrations remain**.                        |
| **Conduct Report (Quests)**      | Review of Member behavior on Quests (reasons: `CONDUCT_ABANDONED`, `CONDUCT_OUT_OF_SCOPE`, `CONDUCT_NO_SHOW`). Decisions: `CONDUCT_REPORT_PENDING`, `CONDUCT_REPORT_UPHELD` (creates Misconduct strike), `CONDUCT_REPORT_DISMISSED`.     | `adminConductReport` persists the retained Quest conduct record. Admin queue, detail, dismissal, and upheld decisions are implemented by `admin-report.route.ts` and `admin-report.service.ts`; an upheld decision writes an Admin Action and penalty record and enqueues an Android Push delivery in the same transaction. The Push module registers Devices, stores delivery state, retries transient FCM failures, and sends a data-only FCM payload with a stable event key. | **Partial — Android client display/deduplication and runtime Push credentials remain unavailable**. |
| **Member Ban & Penalty Ladders** | Two penalty ladders (Misconduct ladder and Low Average Review ladder). Results: Red Flag (7 days), Temporary Ban (7 days), Permanent Ban. Backed by immutable `memberPenaltyRecord` audit table. Auth guard checks active ban.           | `auth_user.bannedUntil` and `redFlagExpiresAt`, immutable penalty records, PC-12/PC-13 and Review crossing rules, Report Case reversal, Auth guards, Red Flag Quest gates, automatic Wallet freezes, and temporary-ban expiry restoration are implemented by the Admin Penalty, Auth, and Quest services.                                                                                                                                                                        | **Implemented for Issues 598 and 607**.                                                             |
...
| Member Ban columns and guard        | Target requires `auth_user.banned_until` and `auth_user.red_flag_expires_at` for O(1) guard checks, backed by `memberPenaltyRecord`. Auth guard rejects active bans.                                        | `auth_user` stores both projections; Auth guards reject active bans and read permanent bans from `memberPenaltyRecord`. Red Flag gates cover Candidate applications, FCFS joins, and Quest publishing.                                                                                                                                           | **Resolved for Issue 598**.                                                             |
| Member Penalty Record table         | Target requires immutable `memberPenaltyRecord` table storing strikes, sequence numbers, results, and linked reversing rows.                                                                                | The schema and forward migrations create the immutable record table, source uniqueness, and reversal links; Admin and Quest services write both ladders and Report Case reversals.                                                                                                                                                               | **Resolved for Issue 598**.                                                             |
| Moderation tables                   | Target requires Report Case, Reporter Entry, Evidence Reference, and Moderation Decision tables. The shared Admin Action table is tracked separately below.                                                 | `admin_report_cases`, `admin_reporter_entries`, `admin_evidence_references`, and `admin_moderation_decisions` exist in `admin.schema.ts` with PostgreSQL constraints and indexes. Student intake creates the Report Case and Message Evidence Reference; Admin queue, evidence access, decisions, strike creation, and reversal are implemented. | **Partial** — Push notification and retention integrations remain.                      |
| Conduct Report table                | Target requires Conduct Report table keyed to Quest and reported Member with fixed reason enums and Admin decisions.                                                                                        | `adminConductReport` and Admin queue/detail/decision routes are implemented; an upheld decision records its Admin Action, Misconduct penalty, and idempotent Push Delivery in one transaction. The Push worker retries transient direct FCM failures.                                                                                            | **Partial** — Android client display/deduplication and runtime Push credentials remain. |
...
| Wallet Auto-freeze on ban           | Target requires auto-freezing a Wallet when a ban starts and restoring to `ACTIVE` when a temporary ban expires.                                                                                            | Member Penalty applies the automatic freeze in the same transaction as the ban record. The bounded expiry worker rebuilds ban projections and restores `ACTIVE` only when the latest Wallet status history records an automatic ban freeze; it leaves discretionary `FROZEN` and `SUSPENDED` statuses unchanged.                                 | **Resolved for Issue 607** — `admin-rulebook.md` §§4, 6.                                |
...
- [x] Add `auth_user.banned_until` and `auth_user.red_flag_expires_at` columns to `auth_user`.
- [x] Create the `memberPenaltyRecord` table with strike sequence, penalty result, and linked reversing rows.
- [x] Create the shared immutable `adminAction` table and transaction-aware writer.
- [x] Create the remaining Trust & Safety moderation tables: `reportCase`, `reporterEntry`, `evidenceReference`, and `moderationDecision`.
- [x] Add a generated Report Case public sequence for stable `RPT-######` display IDs.
- [x] Create the `conductReport` table keyed to Quest and reported Member.
- [x] Add Student Message Report submission and own-status routes with Message visibility checks.
...
- [x] Extend Member auth guard to reject sessions while a ban is active (evaluating `auth_user.banned_until` and permanent ban rows in `memberPenaltyRecord`).
- [x] Apply PC-12/PC-13 and Review ladder rules; gate Red Flag Members from Candidate applications, FCFS joins, and new Quest publishing.
...
- [x] Implement auto-freeze for Wallet on ban creation, and auto-unfreeze when temporary ban expires.
- [x] Add Trust & Safety Admin routes: Evidence Reference retrieval and case resolution (`REPORT_CASE_DISMISSED`, `REPORT_CASE_HIDDEN`, `REPORT_CASE_RESTORED`). The shared `recordEvidenceAccess` writer is used by the case-scoped Evidence route; Admin Quest detail is not evidence, because `admin-trust-safety-contract.md` scopes evidence to Message content and Attachments reached through an Evidence Reference. Push notification and retention-worker integrations remain separate work.
- [x] Add Admin Conduct Report routes, write a Misconduct penalty, and enqueue an Android Push when an Admin upholds a Conduct Report. Product report intake routing remains separate.
- [x] Implement strike reversal logic for `REPORT_CASE_RESTORED` in `memberPenaltyRecord`.
