...
- Work starts when the required Worker explicitly calls the Start Work command
  between `startTime` and `dueAt`: `/api/v1/quests/:questId/start-work` for
  legacy Quests or `/api/v2/quests/:questId/start-work` for v2 Quests. The Quest
  enters `QUEST_IN_PROGRESS` only after every required starter acts.
...
| **Quest lifecycle worker** | Fails assigned v1/v2 Quests missing required Start Work at `dueAt`; also runs existing v2 proof failure, legacy overdue dispute, open-Quest cancellation, invitation/edit expiry, and due-proof approval paths. | `quest-lifecycle.worker.ts`                                                                                                                | **Start Work due failure implemented for v1/v2**; existing legacy proof/confirmation behavior still conflicts with the target. |
...
This table is the shipped server model. A transition marked _blocked at runtime_
...
| `QUEST_ASSIGNED`                                          | Required Worker calls the API-versioned Start Work route between `startTime` and `dueAt`; GROUP + FCFS waits for every Active Worker, while GROUP + CANDIDATE waits for its Team Leader. | `QUEST_IN_PROGRESS`                     | Persists `startedAt` on the acting Assignment; transition occurs only after all required starters act.                                                      | **Implemented for v1 and v2** — `shared/quest-start-work.service.ts`.                                                                                                            |
| `QUEST_ASSIGNED`                                          | Lifecycle worker reaches `dueAt` before a required starter acts.                                                                                                                         | `QUEST_FAILED`                          | Missing FCFS starters become incomplete; a missed Candidate GROUP Team Leader start makes every team Assignment incomplete; held Quest Escrow is preserved. | **Implemented for v1 and v2** — legacy due failure and `failDueAtQuestV2Proofs`.                                                                                                 |
...
| `QUEST_ASSIGNED`                                          | Lifecycle worker reaches `dueAt` before a required starter acts.                                                                                                                         | `QUEST_FAILED`                          | Active Assignments become incomplete; held Quest Escrow is preserved.                                                                                       | **Implemented for v1 and v2**.                                                                                                                                                   |
| `QUEST_IN_PROGRESS`, `QUEST_SUBMITTED`, or `QUEST_REWORK` | Lifecycle worker reaches `dueAt` and a required proof or confirmation is incomplete.                                                                                                     | `QUEST_DISPUTED`                        | Escrow remains reserved.                                                                                                                                    | **Code Only / Conflict** — existing legacy proof and confirmation failure behavior.                                                                                              |
...
stateDiagram-v2
    ...
    QUEST_ASSIGNED --> QUEST_FAILED: v1/v2 dueAt missing Start Work
...
3. A Worker joins an FCFS Quest, or the Hirer selects a Candidate or Team. The
   ...
   `QUEST_ASSIGNED`.
4. The required Worker explicitly starts work between `startTime` and `dueAt`.
   The API-versioned `/api/v1/quests/:questId/start-work` or
   `/api/v2/quests/:questId/start-work` command records that Assignment's
   `startedAt`; the Quest enters `QUEST_IN_PROGRESS` only after all required
   starters act. The lifecycle worker never starts assigned Quests.
...
sequenceDiagram
    ...
    R->>Q: Start Work command during the startTime–dueAt window
    Q-->>R: Assignment startedAt / QUEST_ASSIGNED or QUEST_IN_PROGRESS
...
### Accepted target rules and implementation status
...
| System and Push                | Quest or membership event.                                                                                                                      | Target defines immutable System Messages and Android-only direct FCM delivery with idempotent retries. The Push module supports Android Device registration, delivery state, retries, and a data-only Admin Conduct Report uphold Event with a stable `eventKey`; the Android client must display and deduplicate that key. Quest and Membership events do not yet enqueue deliveries. | Target §§System Message, Push; ADRs `0017`, `0018`                                                                       | **Partial** — Android client delivery handling, Quest and Membership Push, System Messages, and Audit integration remain Docs Only. |
...
| **Overdue start, proof, or confirmation** | Missing Start Work on an assigned Quest fails it at `dueAt`; legacy missing proof/confirmation moves to `QUEST_DISPUTED`. | Missing required action at `dueAt` moves to `QUEST_FAILED`. **Start Work aligned; proof/confirmation conflict remains.** |
...
- [x] Create Android Push Device and delivery tables for the Admin Conduct Report uphold Event. Quest and Membership event delivery remains open.

### Code and workflow

- [ ] Implement 10-minute all-Active-Worker Quest Edit protocol in `QUEST_ASSIGNED` without pausing Quest state.
- [x] Implement Start Work protocol from `startTime` through `dueAt` with starter matrix (Single: Worker; Candidate Group: Team Leader; FCFS Group: all Active Workers) on both versioned routes; automatic worker start has been removed.
...
- [x] Wire direct FCM data-only Push delivery with retryable state and a stable `eventKey` for upheld Conduct Reports. Android client display/deduplication and Quest and Membership event coverage remain open.
...
- `src/modules/quest/lifecycle/quest-lifecycle.worker.ts`
- `src/modules/quest/shared/quest-start-work.service.ts`
- `src/modules/quest/v2/proof/quest-proof-v2.service.ts`
