...
const createWorkConversation = async (): Promise<{
  ...
  const result = await db.transaction((transaction) =>
    createWorkChatMembershipWriter().applyQuestTransition(transaction, {
...
const createValidatedAttachments = async (
  ...
  await db.insert(chatAttachment).values(
    attachments.map(({ attachmentId, fileId, index }) => ({
      id: attachmentId,
      conversationId,
...
describe('Work Chat Member API', () => {
  it('requires Member authentication for every documented route', async () => {
    ...
    unauthorizedAttachment.set(
      'file',
      new File([new Uint8Array([1])], 'file.png', { type: 'image/png' })
    );
    const responses = await Promise.all([
      workChatApp.handle(new Request('http://localhost/api/v1/chat/conversations')),
      workChatApp.handle(
        new Request(`http://localhost/api/v1/chat/conversations/${randomUUID()}/messages`)
      ),
      workChatApp.handle(
        new Request(`http://localhost/api/v1/chat/conversations/${randomUUID()}/participants`)
      ),
      workChatApp.handle(
        new Request(`http://localhost/api/v1/chat/conversations/${randomUUID()}/attachments`, {
          method: 'POST',
      ...
      workChatApp.handle(
        new Request(
          `http://localhost/api/v1/chat/conversations/${randomUUID()}/attachments/${randomUUID()}/link`
        )
      ),
      workChatApp.handle(
        new Request(
          `http://localhost/api/v1/chat/conversations/${randomUUID()}/attachments/${randomUUID()}`,
          {
            method: 'DELETE',
          }
        )
      ),
      requestJson('POST', `/api/v1/chat/conversations/${randomUUID()}/read`, {
  ...
  it('exposes the documented Work Conversation operations with Member security', async () => {
    ...
    expect(
      document.paths['/api/v1/chat/conversations/{conversationId}/messages']?.post?.responses?.[
        '429'
      ]
    ).toBeDefined();
  });
  ...
  it('allows the accepted Hirer and Worker to use the Conversation and denies a non-member', async () => {
    ...
    const workerListBody = (await workerList.json()) as {
      data: { items: Array<{ id: string; quest: { id: string } }> };
    };
    ...
    const history = await workChatApp.handle(
    ...
    uploadBody.set(
      'file',
      new File([new Uint8Array([1, 2, 3])], 'uploaded.png', { type: 'image/png' })
    );
    const uploaded = await workChatApp.handle(
      new Request(`http://localhost/api/v1/chat/conversations/${conversationId}/attachments`, {
        method: 'POST',
        headers: { 'x-member-id': workerId },
        body: uploadBody,
      })
    ...
    const attachmentMessage = await requestJson(
      'POST',
      `/api/v1/chat/conversations/${conversationId}/messages`,
      {
    ...
    const link = await workChatApp.handle(
      new Request(
    ...
    const replayBody = (await replay.json()) as {
      data: { message: { id: string; sequence: number } };
    };
    ...
    const attachmentSent = await requestJson(
      'POST',
      `/api/v1/chat/conversations/${conversationId}/messages`,
      {
    ...
    expect(attachmentSentBody.data.message.attachments.map(({ id }) => id)).toEqual(
      requestedAttachmentIds
    );

    const attachmentReplay = await requestJson(
      'POST',
      `/api/v1/chat/conversations/${conversationId}/messages`,
      {
        clientMessageId: 'worker-message-with-attachments',
    ...
    expect(attachmentReplayBody.data.message.attachments.map(({ id }) => id)).toEqual(
      requestedAttachmentIds
    );

    const newestPage = await workChatApp.handle(
      new Request(`http://localhost/api/v1/chat/conversations/${conversationId}/messages?limit=1`, {
        headers: { 'x-member-id': workerId },
      })
    );
    const newestPageBody = (await newestPage.json()) as {
    ...
    const olderPage = await workChatApp.handle(
      new Request(
    ...
    const nonMemberHistory = await workChatApp.handle(
      new Request(`http://localhost/api/v1/chat/conversations/${conversationId}/messages`, {
    ...
    const nonMemberSend = await requestJson(
      'POST',
      `/api/v1/chat/conversations/${conversationId}/messages`,
      {
    ...
    const nonMemberParticipants = await workChatApp.handle(
      new Request(`http://localhost/api/v1/chat/conversations/${conversationId}/participants`, {
        headers: { 'x-member-id': otherMemberId },
      })
    );
    ...
    const terminalSend = await requestJson(
      'POST',
      `/api/v1/chat/conversations/${conversationId}/messages`,
      {
    ...
    const terminalHistory = await workChatApp.handle(
      new Request(`http://localhost/api/v1/chat/conversations/${conversationId}/messages`, {
        headers: { 'x-member-id': workerId },
      })
    );
  ...
  it('validates message content and mutually exclusive history cursors', async () => {
    ...
    const emptyMessage = await requestJson(
      'POST',
      `/api/v1/chat/conversations/${conversationId}/messages`,
      {
    ...
    const invalidHistory = await workChatApp.handle(
      new Request(
  ...
  it('enforces the per-Minute Message and Attachment limits', async () => {
    ...
    for (let index = 0; index < 30; index += 1) {
      const response = await requestJson(
        'POST',
        `/api/v1/chat/conversations/${messageFixture.conversationId}/messages`,
        {
    ...
    const messageLimited = await requestJson(
      'POST',
      `/api/v1/chat/conversations/${messageFixture.conversationId}/messages`,
      {
    ...
    const attachmentIds = await createValidatedAttachments(
      attachmentFixture.conversationId,
      workerId,
      11
    );
    const firstAttachments = await requestJson(
      'POST',
      `/api/v1/chat/conversations/${attachmentFixture.conversationId}/messages`,
      {
    ...
    const secondAttachments = await requestJson(
      'POST',
      `/api/v1/chat/conversations/${attachmentFixture.conversationId}/messages`,
      {
    ...
    const attachmentLimited = await requestJson(
      'POST',
      `/api/v1/chat/conversations/${attachmentFixture.conversationId}/messages`,
      {
    ...
    const [preparedAttachment] = await db
      .select({ status: chatAttachment.status })
      .from(chatAttachment)
  ...
  it('discards an uncommitted attachment and removes its private object', async () => {
    ...
    const discarded = await workChatApp.handle(
      new Request(
    ...
    const [pendingCleanup] = await db
      .select({ objectDeletedAt: chatAttachment.objectDeletedAt })
      .from(chatAttachment)
    ...
    const [storedAttachment] = await db
      .select({ status: chatAttachment.status, deletedAt: chatAttachment.deletedAt })
      .from(chatAttachment)
      .where(eq(chatAttachment.id, uploadedBody.data.attachment.id));
