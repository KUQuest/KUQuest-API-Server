...
import type { AcceptedWorker, QuestWorkChatMembershipTransition } from '@/modules/quest';
...
describe('WorkChatMembershipWriter', () => {
  it('creates one Conversation, opens windows, records System Messages, and deduplicates retries', async () => {
    ...
    const created = await db.transaction((transaction) =>
      writer.applyQuestTransition(transaction, first)
    );
    ...
    await expect(
      db.transaction((transaction) =>
        writer.applyQuestTransition(transaction, {
          ...first,
    ...
    const memberships = await db
      .select()
      .from(chatMembership)
      .where(eq(chatMembership.conversationId, created.conversationId));
  ...
  it('rejects an Assignment whose Worker does not match the transition', async () => {
    ...
    await expect(
      db.transaction((transaction) => writer.applyQuestTransition(transaction, malformed))
  ...
  it('closes the Worker window and makes a terminal Conversation read-only once', async () => {
    ...
    const result = await db.transaction((transaction) =>
      writer.applyQuestTransition(transaction, terminal)
    );
