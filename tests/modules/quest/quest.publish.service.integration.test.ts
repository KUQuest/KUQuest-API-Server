...
describe('Quest publishing service', () => {
  ...
  it('reserves Quest Escrow before changing a valid Draft to Open', async () => {
    ...
    expect(reservation?.totalReservedSatang).toBe(preview.escrowRequirement * 100);
    const wallet = await readTestWallet(hirerId);
    expect(wallet).toMatchObject({
      spendingBalanceSatang: beforeWallet.spendingBalanceSatang - 51_000,
      fundingReservedSatang: beforeWallet.fundingReservedSatang + 51_000,
    });
