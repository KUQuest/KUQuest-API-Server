...
describe('Quest Review HTTP contract', () => {
  it.each([
    ...
    'returns the shared validation envelope for invalid %s input',
  ...
  it.each([
    ...
    'requires Member authentication for %s Review mutations',
    async (method, path, body) => {
      const response = await request(method, path, body);
  ...
  it('publishes the Review create, edit, and no-delete operations', async () => {
    ...
    expect(document.paths[`/api/v1/quests/{questId}/reviews`]?.post?.operationId).toBe(
      'createQuestReview'
    );
    expect(document.paths[`/api/v1/quests/{questId}/reviews/{reviewId}`]?.patch?.operationId).toBe(
      'updateQuestReview'
    );
    expect(document.paths[`/api/v1/quests/{questId}/reviews/{reviewId}`]?.delete?.operationId).toBe(
      'deleteQuestReview'
    );
  });
});
