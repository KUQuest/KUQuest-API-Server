...
const createV2Quest = async (input: {
  ...
  const headcount =
    input.headcount ?? (participation === 'SINGLE' ? 1 : Math.max(2, workers.length));
  ...
  await db.insert(quest).values({
    ...
    questFundingTotalSatang:
      input.status === 'QUEST_DRAFT' ? null : rewardSatang + platformFeeSatang,
    ...
    questEscrowSatang:
      input.status === 'QUEST_DRAFT' ? null : (rewardSatang + platformFeeSatang) * headcount,
...
const selectCandidateTeam = async (
  questId: string,
  leaderId: string,
  workerIdsForTeam: string[]
) => {
...
const completeUnderfilledAllocation = async (
  ...
  await db.insert(questV2UnderfilledConsent).values(
    workerIdsForAllocation.map((workerId, index) => {
      const assignment = assignments.find(
        ({ workerId: assignmentWorkerId }) => assignmentWorkerId === workerId
      );
...
describe('Quest API v2 Hirer cancellation', () => {
  ...
  it('requires Idempotency-Key before authentication and rejects a non-owner', async () => {
    ...
    const response = await request(
      storedQuestId,
      otherMemberId,
      `cancel-v2-not-owner-${storedQuestId}`
    );
    ...
    expect(
      (
        await db
          ...
          .from(quest)
          .where(eq(quest.id, storedQuestId))
      )[0]?.status
    ).toBe('QUEST_DRAFT');
  });
  ...
  it('cancels a Draft without money movement and closes the Quest atomically', async () => {
    ...
    ).toEqual([{ action: 'QUEST_STATE_CHANGED', resourceType: 'QUEST' }]);
      ...
      expect(
        (
          await db
            ...
            .from(questAssignment)
            .where(eq(questAssignment.questId, questId))
        ).map(({ status: assignmentState }) => assignmentState)
      ).toEqual(workers.map(() => 'ASSIGNMENT_CANCELLED'));
      expect(
        transitions
          .filter(({ questId: transitionQuestId }) => transitionQuestId === questId)
          .map(({ type }) => type)
      ).toEqual(
        workers.length === 0
          ? ['questBecameReadOnly']
  ...
  it('uses the active Worker allocation for GROUP + FCFS and pays only the Team Leader for GROUP + CANDIDATE', async () => {
    ...
    expect((await fcfsResponse.json()).data).toMatchObject({
      paidSatang: 600,
      refundedSatang: 2_463,
    });
    ...
    const candidateResponse = await request(
      ...
      hirerId,
      `cancel-v2-candidate-${candidateAssignedQuestId}`
    );
    expect(candidateResponse.status).toBe(200);
    expect((await candidateResponse.json()).data).toMatchObject({
      paidSatang: 400,
    ...
    const inProgressResponse = await request(
      ...
      hirerId,
      `cancel-v2-candidate-progress-${candidateInProgressQuestId}`
    );
    expect(inProgressResponse.status).toBe(200);
    expect((await inProgressResponse.json()).data).toMatchObject({
      paidSatang: 2_000,
      refundedSatang: 0,
    });
  ...
  it('rejects terminal States and preserves the Quest, while a reused key fails canonically', async () => {
    ...
    const reusedByOtherMember = await request(
      openQuestId,
      otherMemberId,
      'cancel-v2-cross-request'
    );
