...
import type { QuestStatus } from '@/modules/quest/shared';
...
const adminRequest = (path: string, body: unknown, headers: HeadersInit = {}) =>
  app.handle(
...
const asMember = (memberId: string = workerId) =>
  spyOn(auth.api, 'getSession').mockImplementation((async () => ({
    user: { id: memberId },
    session: { userId: memberId },
  })) as never);

const workerRequest = (path: string, headers: HeadersInit = {}, body?: unknown) =>
  app.handle(
    new Request(`http://localhost${path}`, {
      method: 'POST',
...
const memberGet = (path: string) =>
  app.handle(new Request(`http://localhost${path}`, { method: 'GET' }));

const hideQuest = (questId: string, version: number) =>
  adminRequest(
...
const hideOpenQuest = async (questId: string) => {
  ...
  const [row] = await db
    .select({ hiddenAt: quest.hiddenAt, questStatus: quest.questStatus })
    .from(quest)
...
describe('Admin Quest moderation commands', () => {
  it('publishes Admin command paths with Admin security', async () => {
    ...
    expect(document.paths['/api/v1/admin/quests/{questId}/hide']?.post?.operationId).toBe(
      'hideAdminQuest'
    );
    expect(document.paths['/api/v1/admin/quests/{questId}/restore']?.post?.operationId).toBe(
      'restoreAdminQuest'
    );
    expect(document.paths['/api/v1/admin/quests/{questId}/terminate']?.post?.operationId).toBe(
      'terminateAdminQuest'
    );
    expect(document.paths['/api/v1/admin/quests/{questId}/hide']?.post?.security).toEqual([
      { betterAuthSession: [] },
    ]);
  });
  it('hides an active Quest without changing lifecycle, Assignment, Wallet, or Work Conversation', async () => {
    ...
    expect(
      await db
        ...
        .from(quest)
        .where(eq(quest.id, questId))
    ).toEqual([{ status: 'QUEST_OPEN', version: 2 }]);
    expect(
      await db
        .select({ action: adminAction.action, resourceType: adminAction.resourceType })
        .from(adminAction)
        .where(and(eq(adminAction.adminId, adminId), eq(adminAction.resourceId, questId)))
    ).toEqual([{ action: 'QUEST_HIDE', resourceType: 'quest' }]);
  });
  ...
  it('hides ASSIGNED and IN_PROGRESS Quests without changing lifecycle state', async () => {
    ...
    const responses = await Promise.all([
      adminRequest(
        `/api/v1/admin/quests/${assignedQuestId}/hide`,
        { reasonCode: 'POLICY_REVIEW' },
        {
      ...
      adminRequest(
        `/api/v1/admin/quests/${inProgressQuestId}/hide`,
        { reasonCode: 'POLICY_REVIEW' },
        {
    ...
    expect(
      await db
        .select({ status: quest.questStatus, version: quest.version })
        .from(quest)
        .where(eq(quest.id, assignedQuestId))
    ).toEqual([{ status: 'QUEST_ASSIGNED', version: 2 }]);
    expect(
      await db
        .select({ status: quest.questStatus, version: quest.version })
        .from(quest)
        .where(eq(quest.id, inProgressQuestId))
    ).toEqual([{ status: 'QUEST_IN_PROGRESS', version: 2 }]);
  });
  ...
  it('hides every remaining non-terminal Quest state without changing lifecycle state', async () => {
    ...
    const rows = await Promise.all(
      questIdsByState.map(async (questId) => {
        const [row] = await db
          .select({ status: quest.questStatus, hiddenAt: quest.hiddenAt, version: quest.version })
          .from(quest)
          .where(eq(quest.id, questId));
        return row;
      })
  ...
  it('refuses to hide a terminal Quest', async () => {
    ...
    expect(
      await Promise.all(responses.map(async (response) => (await response.json()).error.code))
    ).toEqual(states.map(() => 'QUEST_ACTION_NOT_ALLOWED'));
    const rows = await Promise.all(
      questIdsByState.map(async (questId) => {
        const [row] = await db
          .select({ hiddenAt: quest.hiddenAt, version: quest.version })
          .from(quest)
          .where(eq(quest.id, questId));
        return row;
      })
  ...
  it('pays the whole ASSIGNED cancellation share to the Team Leader of a GROUP Candidate Quest', async () => {
    ...
    await db.insert(questAssignment).values(
      roster.map(({ assignmentId, workerId: memberId }) => ({
        id: assignmentId,
        questId,
        workerId: memberId,
  ...
  it('replays a Hide command and rejects stale or reused keys without another effect', async () => {
    ...
    const reused = await adminRequest(
      `/api/v1/admin/quests/${questId}/hide`,
  ...
  it('restores an eligible hidden Quest without requiring free-form reason text', async () => {
    ...
    const body = (await response.json()) as {
      data: { resourceSummary: Record<string, unknown>; resourceVersion: number };
    };
  ...
  it('keeps an ineligible hidden Quest hidden after its start time', async () => {
    ...
    expect(
      await db
        ...
        .from(quest)
        .where(eq(quest.id, questId))
    ).toEqual([{ hiddenAt: expect.any(Date), version: 1 }]);
  });
  ...
  it('keeps a hidden overlay when termination reaches a terminal state', async () => {
    ...
    const body = (await hideAtTerminal.json()) as {
      data: { resourceSummary: Record<string, unknown> };
    };
  ...
  it('terminates through Quest, Wallet, Assignment, Work Conversation, and Admin Action owners', async () => {
    ...
    expect(
      await db
        ...
        .from(chatConversation)
        .where(eq(chatConversation.questId, questId))
    ).toEqual([{ readOnlyAt: expect.any(Date), archivedAt: expect.any(Date) }]);
  });
  ...
  it('refuses a Candidate Team once an Admin has hidden the Quest', async () => {
    ...
    try {
      response = await workerRequest(
        `/api/v1/quests/${questId}/teams`,
        {},
        { name: 'Hidden Quest Team' }
      );
    } finally {
      session.mockRestore();
    }
  ...
  it('refuses a Team invitation response once an Admin has hidden the Quest', async () => {
    ...
    await db
      .insert(questTeam)
      .values({ id: teamId, questId, leaderId: teamLeaderId, name: 'Hidden Quest Team' });
    ...
    expect(
      await db
        ...
        .where(and(eq(questTeamMember.teamId, teamId), eq(questTeamMember.userId, workerId)))
    ).toEqual([]);
  });
  ...
  it('refuses a new Team invitation once an Admin has hidden the Quest', async () => {
    ...
    expect(
      await db
        ...
        .where(eq(questTeamInvitation.teamId, teamId))
    ).toEqual([]);
  });
  ...
  it('hides a pending invitation from Member reads once an Admin has hidden the Quest', async () => {
    ...
    try {
      listedBefore = (await (
        await memberGet('/api/v1/quests/invitations')
      ).json()) as typeof listedBefore;
    } finally {
      before.mockRestore();
    }
