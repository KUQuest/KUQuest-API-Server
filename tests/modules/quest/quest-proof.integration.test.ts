...
describe('Quest Proof HTTP contract', () => {
  ...
  it('requires Member authentication for proof commands', async () => {
    const responses = await Promise.all([
      ...
      request('POST', `/api/v1/quests/${questId}/proof/${proofId}/review`, {
  ...
  it('publishes proof routes in OpenAPI', async () => {
    ...
    expect(document.paths['/api/v1/quests/{questId}/proof']?.post?.operationId).toBe(
      'submitQuestProof'
    );
    expect(
      document.paths['/api/v1/quests/{questId}/proof/{proofId}/review']?.post?.operationId
    ).toBe('reviewQuestProof');
  });
});
