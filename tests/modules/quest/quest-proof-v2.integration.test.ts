...
const request = (method: string, path: string, headers: HeadersInit = {}, body?: BodyInit) =>
...
describe('Quest Proof API v2 contract', () => {
  it('requires Idempotency-Key on every state-changing v2 Proof operation before authentication', async () => {
    const requests = [
      request(
        ...
        JSON.stringify({ description: 'done' })
      ),
      request(
        'PATCH',
  ...
  it('documents all seven v2 operations with canonical Proof statuses and no retired vocabulary', async () => {
    ...
    const operations = [
      ['/api/v2/quests/{questId}/proof-submissions', 'post', 'createQuestV2ProofSubmission'],
      ['/api/v2/quests/{questId}/proof-submissions', 'get', 'listQuestV2ProofSubmissions'],
      [
        '/api/v2/quests/{questId}/proof-submissions/{proofSubmissionId}',
        'patch',
    ...
    const createBody =
      document.paths['/api/v2/quests/{questId}/proof-submissions']?.post?.requestBody;
