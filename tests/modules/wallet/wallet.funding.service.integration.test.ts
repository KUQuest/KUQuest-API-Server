...
describe('Funding Reservation service', () => {
  it('reserves Spending atomically through the caller transaction', async () => {
    ...
    expect(
      await db.select().from(walletWallet).where(eq(walletWallet.userId, ownerUserId))
    ).toMatchObject([{ spendingBalanceSatang: 6_000, fundingReservedSatang: 4_000 }]);
    ...
    expect(postings.map(({ amountSatang }) => amountSatang).sort((a, b) => a - b)).toEqual([
      -4_000, 4_000,
    ]);
  });
  ...
  it('replays reserve, increase, and release operation results', async () => {
    ...
    const reserved = await db.transaction((transaction) =>
      reserveSpending(transaction, reserveInput)
    );
    const reserveReplay = await db.transaction((transaction) =>
      reserveSpending(transaction, reserveInput)
    );
    ...
    const increased = await db.transaction((transaction) =>
      increaseFundingReservation(transaction, increaseInput)
    );
    const increaseReplay = await db.transaction((transaction) =>
      increaseFundingReservation(transaction, increaseInput)
    );
    ...
    const released = await db.transaction((transaction) =>
      releaseFundingReservation(transaction, releaseInput)
    );
    const releaseReplay = await db.transaction((transaction) =>
      releaseFundingReservation(transaction, releaseInput)
    );
    expect(releaseReplay).toEqual(released);
    expect(
      await db.select().from(walletWallet).where(eq(walletWallet.userId, payerUserId))
  ...
  it('increases an active reservation with additional Spending', async () => {
    ...
    const increased = await db.transaction((transaction) =>
      increaseFundingReservation(transaction, {
        ownerUserId,
    ...
    expect(
      await db.select().from(walletWallet).where(eq(walletWallet.userId, ownerUserId))
    ).toMatchObject([{ spendingBalanceSatang: 4_500, fundingReservedSatang: 5_500 }]);
  });
  ...
  it('partially settles to recipient Earnings plus a Platform Fee', async () => {
    ...
    expect(
      await db.select().from(walletWallet).where(eq(walletWallet.userId, recipientUserId))
  ...
  it('continues using the reservation policy snapshot after a newer policy becomes effective', async () => {
    ...
    const existingPolicies = await db
      .select({
        effectiveFrom: paymentMoneyPolicyRevision.effectiveFrom,
        effectiveUntil: paymentMoneyPolicyRevision.effectiveUntil,
      })
      .from(paymentMoneyPolicyRevision);
    let windowStart = 0;
    for (let attempt = 0; attempt < 100; attempt += 1) {
      const candidateStart =
        Date.UTC(2020, 0, 1) +
        Math.floor(Math.random() * (Date.UTC(2024, 0, 1) - Date.UTC(2020, 0, 1) - 3_600_000));
      const candidateEnd = candidateStart + 7_200_000;
      const overlaps = existingPolicies.some(
        (policy) =>
    ...
    const reservation = await withCurrentDate(new Date(windowStart + 1_800_000), () =>
      db.transaction((transaction) =>
        reserveSpending(transaction, {
          ownerUserId: payerUserId,
          callerScope: 'generic-workflow',
    ...
    await withCurrentDate(new Date(windowStart + 5_400_000), () =>
      db.transaction((transaction) =>
        increaseFundingReservation(transaction, {
          ownerUserId: payerUserId,
          reservationId: reservation.id,
    ...
    const settlement = await withCurrentDate(new Date(windowStart + 5_400_000), () =>
      db.transaction((transaction) =>
        settleFundingReservation(transaction, {
          ownerUserId: payerUserId,
          reservationId: reservation.id,
  ...
  it('replays the same settlement and rejects conflicting key reuse', async () => {
    ...
    const first = await db.transaction((transaction) =>
      settleFundingReservation(transaction, input)
    );
  ...
  it('serializes concurrent settlements so they cannot oversettle', async () => {
    ...
    const settle = (settlementReference: string) =>
      db.transaction((transaction) =>
        settleFundingReservation(transaction, {
          ownerUserId: payerUserId,
          reservationId: reservation.id,
  ...
  it('rejects reservation overspend and non-active Wallet commitments without partial effects', async () => {
    ...
    expect(
      await db
        .select()
        .from(walletFundingReservation)
        .where(eq(walletFundingReservation.callerReference, callerReference))
    ).toHaveLength(0);
    expect(
      await db.select().from(walletWallet).where(eq(walletWallet.userId, payerUserId))
  ...
  it('enforces Funding Reservation ownership and immutability in PostgreSQL', async () => {
    ...
    const releasable = await db.transaction((transaction) =>
      reserveSpending(transaction, {
        ownerUserId: payerUserId,
        callerScope: 'generic-workflow',
