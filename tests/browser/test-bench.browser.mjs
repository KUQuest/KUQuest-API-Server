...
const quest = {
  ...
  title: 'Design a KU poster',
  state: 'QUEST_DRAFT',
  mode: 'FIRST_COME_FIRST_SERVED',
  participation: 'SINGLE',
  proofRequired: false,
  dueAt: '2026-09-06T10:00:00+07:00',
};
const conversation = (id, readOnly) => ({
  id,
  quest: {
    id: quest.id,
    title: `Poster ${id}`,
    status: readOnly ? 'QUEST_COMPLETED' : 'QUEST_IN_PROGRESS',
  },
  readOnly,
});
const message = {
  id: 'm1',
  kind: 'USER',
  sender: { id: 'account-1', displayName: 'Hirer' },
  text: 'Hello Worker',
...
await page.route('**/api/**', async (route) => {
  ...
  let data;
  let status = 200;
  if (path.includes('/staging/test-auth/sign-in/')) {
    member = path.split('/').at(-1);
    data = {};
  } else if (path === '/api/auth/get-session')
    data = member
      ? { user: { id: member, email: `${member}@ku.th` }, session: { token: 'SECRET_SESSION' } }
      : null;
  else if (path === '/api/auth/sign-out') {
    member = null;
    data = {};
  } else if (path === '/api/admin/auth/get-session')
    data = admin ? { user: { id: 'admin', email: 'admin@test.local' } } : null;
  else if (path === '/api/admin/auth/sign-in/email') {
    admin = true;
    data = { user: { id: 'admin', email: body.email } };
  } else if (path === '/api/admin/auth/sign-out') {
    admin = false;
    data = {};
    ...
    data = ok({
      wallet: {
        ...
        earningsBalanceSatang: 20000,
        fundingReservedSatang: 0,
        reservedForPayoutsSatang: 0,
      },
    });
  else if (path === '/api/v2/quests' && method === 'POST') {
  ...
  else if (path === '/api/v2/quests') {
    const boardQuest = { ...quest };
    delete boardQuest.state;
    data = ok({ items: [boardQuest], nextCursor: null });
  } else if (path.endsWith('/publish-check'))
    data = ok({
      canPublish: allowPublish,
      blockingReasons: allowPublish ? [] : [{ code: 'TAG_REQUIRED', message: 'Choose a Tag.' }],
      ...
      items: [
        {
          ...
          state: 'ASSIGNMENT_ACTIVE',
          questState: quest.state,
          startedAt: quest.state === 'QUEST_IN_PROGRESS' ? new Date().toISOString() : null,
          createdAt: new Date().toISOString(),
        },
      ],
    });
  } else if (path.endsWith('/completion-confirmation')) {
    completed = true;
    quest.state = 'QUEST_COMPLETED';
    data = ok({ confirmed: true, confirmedAt: new Date().toISOString(), questStatus: quest.state });
  } else if (path.endsWith('/reviews') && method === 'POST')
    data = ok({
      id: `review-${member}`,
      questId: quest.id,
      reviewerId: member,
      revieweeId: member === 'account-1' ? 'account-2' : 'account-1',
      rating: body.rating,
    ...
    data = ok({ items: [conversation('c1', false), conversation('c2', true)], nextCursor: null });
  else if (path.endsWith('/participants'))
    data = ok({ participants: [{ id: member, role: 'HIRER', displayName: 'Hirer' }] });
  else if (path.endsWith('/messages') && method === 'GET')
    data = ok({ items: [message], hasMore: false, nextCursor: null });
  ...
  else if (path === '/api/v1/payouts/quotes')
    data = ok({
      id: 'quote-1',
      payoutDestinationId: 'dest-1',
      receiptSatang: body.receiptSatang,
      maximumDebitSatang: body.receiptSatang + 200,
      expiresAt: new Date(Date.now() + quoteLifetime).toISOString(),
    });
  ...
  else if (path === '/api/local/test/payment')
    data = ok({
      testUserEmail: 'finance-fixture@ku.th',
      wallet: { spendingBalanceSatang: 999999 },
      topUp: {},
    });
  else {
    status = 404;
    data = { success: false, error: { message: `Unhandled fixture: ${method} ${path}` } };
  }
  await route.fulfill({ status, contentType: 'application/json', body: JSON.stringify(data) });
});
const idle = async () => {
  await page.waitForFunction(() => !document.querySelector('.card[data-busy="true"]'));
};
...
const nav = async (key) => {
  ...
  await page.locator(`[data-page="${key}"]`).waitFor({ state: 'visible' });
};
try {
  await page.goto(baseURL);
  await page.waitForFunction(() =>
    document.querySelector('#operation-count').textContent.includes('operations')
  );
  ...
  assert.equal(
    await page
      .locator('#debug-log')
      .textContent()
      .then((text) => text.includes('SECRET_SESSION')),
    false
  );
  ...
  assert.match(
    await page.locator('#simple-flow-receipt').textContent(),
  ...
  await click('default-sign-in');
  console.log(
    'PASS: guided proof-free Quest flow, lifecycle wait, completion, Reviews, endpoint receipt'
  );
  ...
  await nav('chat');
  await click('chat-load-live');
  await page.locator('#conversation-select').selectOption('c2');
  await idle();
  assert.equal(await page.locator('#chat-message-input').isDisabled(), true);
  await page.locator('#conversation-select').selectOption('c1');
  await idle();
  ...
  console.log(
    'PASS: live Conversation selection, read-only state, failed send, deduplicated retry'
  );

  await nav('wallet');
  let releaseWallet;
  walletGate = new Promise((resolve) => {
    releaseWallet = resolve;
  });
  await page.locator('#refresh-wallet').click();
  await page.waitForFunction(
    () => document.querySelector('[aria-labelledby=wallet-heading]').dataset.busy === 'true'
  );
  await nav('chat');
  releaseWallet();
  walletGate = null;
  ...
  console.log(
    'PASS: Payout amount changes, Quote expiry, idempotent submit, separate finance test Member'
  );
  ...
  assert.equal(
    await page.locator('.operation input[name="questId"]').first().inputValue(),
    quest.id
  );
  ...
  await page.waitForFunction(() =>
    document.querySelector('.operation pre').textContent.includes('maximumDebitSatang')
  );
  assert.equal(
    writes.at(-1).body.receiptSatang,
    1500,
    'Explorer must not rescale API Satang fields'
  );
  ...
  for (const width of [1440, 390]) {
    ...
    for (const key of ['member', 'flow', 'quests', 'chat', 'wallet', 'admin', 'explorer', 'log']) {
      ...
      assert.equal(
        ...
        false,
        `Overflow: ${key} at ${width}px`
      );
    }
    await nav('member');
    await page.screenshot({
      path: `/tmp/kuquest-bench-verification/member-${width}.png`,
      fullPage: true,
    });
  }
