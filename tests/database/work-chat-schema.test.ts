...
describe('Work Chat database schema', () => {
  it('stores one retained Work Conversation per Quest', () => {
    ...
    expect(
      config.indexes.some((index) => index.config.name === 'chat_conversation_work_quest_uidx')
    ).toBe(true);
    expect(
      config.indexes.some(
        (index) => index.config.name === 'chat_conversation_candidate_worker_uidx'
      )
    ).toBe(true);
  ...
  it('stores Assignment-scoped inclusive Chat Membership windows', () => {
    ...
    expect(
      config.indexes.some(
        (index) => index.config.name === 'chat_membership_one_active_prospective_worker_uidx'
      )
    ).toBe(true);
  });
