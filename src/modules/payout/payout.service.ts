...
const calculatePayoutTerms = (
  ...
  const maximumTaxSatang = satang(
    Math.ceil((maximumFeeSatang * policy.payoutProviderTaxBps) / 10_000)
  );
  return {
    maximumFeeSatang,
    maximumTaxSatang,
...
export const quotePayout = async (input: PayoutQuoteInput): Promise<PayoutQuote> => {
  ...
  if (!created)
    throw new MoneyDomainError('PAYOUT_QUOTE_CREATE_FAILED', 'Payout Quote could not be created.');
...
const lockedPayoutForAdminDecision = async (transaction: WalletTransaction, payoutId: string) => {
...
export const approvePayoutInTransaction = async (
  ...
  const [updated] = await transaction
    ...
    .returning();
  if (!updated)
    throw new MoneyDomainError('PAYOUT_UPDATE_FAILED', 'Payout approval could not be saved.');
...
export const cancelPayoutInTransaction = async (
  ...
  const [updated] = await transaction
    ...
    .returning();
  if (!updated)
    throw new MoneyDomainError('PAYOUT_UPDATE_FAILED', 'Payout cancellation could not be saved.');
  await transaction.insert(paymentPayoutStatusHistory).values({
    payoutId: payout.id,
    fromStatus: 'PENDING_ADMIN_APPROVAL',
    toStatus: 'CANCELLED',
...
const finalizeProviderResponse = async (
  ...
  db.transaction(async (transaction) => {
    const [record] = await transaction
      ...
      .for('update');
    if (!record) throw new MoneyDomainError('PAYOUT_NOT_FOUND', 'Payout does not exist.');
    if (!['SUBMITTED_TO_PROVIDER', 'PROVIDER_PENDING'].includes(record.payoutStatus))
      return payoutFromRecord(record);
    if (
      response.providerAmountSatang !== record.principalSatang ||
      response.actualDebitSatang !==
        record.principalSatang + response.actualFeeSatang + response.actualTaxSatang ||
      response.actualDebitSatang > record.maximumDebitSatang
    ) {
      throw new PayoutProviderError(
        'PROVIDER_UNCERTAIN',
        'Provider Payout amounts do not match the Quote.'
      );
    }
    const [updated] = await transaction
      .update(paymentPayouts)
    ...
    if (!updated)
      throw new MoneyDomainError(
        ...
        'Payout provider state could not be saved.'
      );
    await transaction.insert(paymentPayoutStatusHistory).values({
      payoutId: record.id,
...
const finalizeUncertain = async (
  ...
  db.transaction(async (transaction) => {
    ...
    const nextStatus =
      record.payoutStatus === 'SUBMITTED_TO_PROVIDER' ? 'PROVIDER_PENDING' : record.payoutStatus;
    const [updated] = await transaction
      .update(paymentPayouts)
      .set({
    ...
    if (!updated)
      throw new MoneyDomainError(
        'PAYOUT_UPDATE_FAILED',
        'Uncertain Payout state could not be saved.'
      );
...
const finalizeFailed = async (
  ...
  db.transaction(async (transaction) => {
    ...
    if (!releaseLedger) {
      throw new MoneyDomainError(
        'PAYOUT_UPDATE_FAILED',
        'Failed Payout reserve could not be released.'
      );
    }
    const [updated] = await transaction
      .update(paymentPayouts)
...
const providerStatusForError = (error: PayoutProviderError) =>
  [error.providerStatus?.toString(), safeProviderCode(error.providerCode)]
    .filter((value): value is string => value !== undefined)
    .join(':') || error.code;

const providerFailureReason = (prefix: string, error: PayoutProviderError) =>
  sanitizePayoutProviderDiagnostic(
    [
      prefix,
      error.providerStatus === undefined ? undefined : `HTTP ${error.providerStatus}`,
      safeProviderCode(error.providerCode)
        ? `Code ${safeProviderCode(error.providerCode)}`
        : undefined,
...
const claimApprovedPayout = async (payoutId: string) =>
  db.transaction(async (transaction) => {
    const [record] = await transaction
      .select()
      .from(paymentPayouts)
      ...
      .for('update');
    if (!record) throw new MoneyDomainError('PAYOUT_NOT_FOUND', 'Payout does not exist.');
    if (record.payoutStatus !== 'SUBMITTED_TO_PROVIDER')
    ...
    if (!claimed)
      throw new MoneyDomainError(
        'PAYOUT_UPDATE_FAILED',
        'Payout could not be claimed by the Worker.'
      );
  ...
  const candidates = await db
    ...
    .where(
      and(
        ...
        or(
          isNull(paymentPayouts.providerSubmissionClaimedAt),
          lt(paymentPayouts.providerSubmissionClaimedAt, staleBefore)
        )
      )
    )
