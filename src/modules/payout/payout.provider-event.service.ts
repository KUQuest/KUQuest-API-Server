...
const providerEventFromRecord = (
  ...
  actualFeeSatang:
    record.providerActualFeeSatang === null ? null : satang(record.providerActualFeeSatang),
  actualTaxSatang:
    record.providerActualTaxSatang === null ? null : satang(record.providerActualTaxSatang),
  actualDebitSatang:
    record.providerActualDebitSatang === null
      ? null
...
const claimEvents = async (
  ...
  for (const candidate of candidates) {
    if (
      ...
      candidate.attemptCount >= payoutProviderEventMaxAttempts
    ) {
...
export const claimPayoutProviderEvents = async (
  ...
  db.transaction((transaction) => claimEvents(transaction, input));
...
const payoutForEvent = async (
  ...
  const conditions = [
    event.internalReference
      ? eq(paymentPayouts.internalReference, event.internalReference)
      : undefined,
    event.providerReference
      ? eq(paymentPayouts.providerReference, event.providerReference)
      : undefined,
  ].filter((condition): condition is NonNullable<typeof condition> => Boolean(condition));
  if (conditions.length === 0) {
    throw new ProviderEventError(
      'PROVIDER_EVENT_NOT_FOUND',
      'Provider event has no Payout reference.'
    );
  }
  ...
  if (records.length !== 1) {
    throw new ProviderEventError(
      'PROVIDER_EVENT_NOT_FOUND',
      'Provider event does not match one Payout.'
    );
  }
  ...
  if (event.internalReference && event.internalReference !== payout.internalReference) {
    throw new ProviderEventError(
      'PROVIDER_EVENT_INVALID',
      'Provider event references a different Payout.'
    );
  }
  if (
    event.providerReference &&
    payout.providerReference &&
    event.providerReference !== payout.providerReference
  ) {
    throw new ProviderEventError(
      'PROVIDER_EVENT_INVALID',
      'Provider event references a different Provider payout.'
    );
  }
  return payout;
};
...
const settlePayout = async (
  ...
  const postings = [
    ...
    {
      accountId: accounts.platformSuspenseId,
      amountSatang: signedSatang(payout.principalSatang + actualTaxSatang),
    },
...
const releasePayoutReserve = async (
  ...
  return createSealedLedgerTransactionInTransaction(transaction, {
    ...
    postings: [
      ...
      {
        accountId: accounts.payoutReserveId,
        amountSatang: signedSatang(-payout.maximumDebitSatang),
      },
    ],
  });
};
...
const reverseSucceededPayout = async (
  ...
  if (!payout.finalLedgerTransactionId) {
    throw new ProviderEventError(
      'PROVIDER_EVENT_INVALID',
      'Succeeded Payout has no ledger transaction to reverse.'
    );
  }
  ...
  if (originalPostings.length === 0) {
    throw new ProviderEventError(
      'PROVIDER_EVENT_INVALID',
      'Succeeded Payout ledger postings are missing.'
    );
  }
...
const applyPayoutOutcomeInTransaction = async (
  ...
  if (
    ...
    facts.providerAmountSatang !== payout.principalSatang
  ) {
    throw new ProviderEventError(
      'PROVIDER_EVENT_INVALID',
      'Provider amount does not match the Payout principal.'
    );
  }

  const providerFacts = providerFactsFor(payout, facts);
  if (facts.normalizedStatus === 'SUCCEEDED') {
    const actualFeeSatang =
      facts.actualFeeSatang ??
      (payout.actualFeeSatang === null ? satang(0) : satang(payout.actualFeeSatang));
    const actualTaxSatang =
      facts.actualTaxSatang ??
      (payout.actualTaxSatang === null ? satang(0) : satang(payout.actualTaxSatang));
    const actualDebitSatang =
      facts.actualDebitSatang ??
      (payout.actualDebitSatang === null
    ...
    if (
      ...
      throw new ProviderEventError(
        'PROVIDER_EVENT_INVALID',
        'Provider Payout amounts do not match the reserve.'
      );
    }
    ...
    const ledger = await settlePayout(
      transaction,
      payout,
      actualFeeSatang,
      actualTaxSatang,
    ...
    if (!ledger)
      throw new MoneyDomainError(
        ...
        'Payout settlement ledger could not be created.'
      );
    const [updated] = await transaction
      .update(paymentPayouts)
      .set({
    ...
    if (!updated)
      throw new MoneyDomainError(
        'PAYOUT_UPDATE_FAILED',
        'Succeeded Payout state could not be saved.'
      );
  ...
  if (facts.normalizedStatus === 'PROVIDER_PENDING') {
    if (
      payout.payoutStatus === 'SUBMITTED_TO_PROVIDER' ||
      payout.payoutStatus === 'PROVIDER_PENDING'
    ) {
      await transaction
        .update(paymentPayouts)
        .set({
  ...
  if (!ledger)
    throw new MoneyDomainError('PAYOUT_UPDATE_FAILED', 'Payout reserve could not be released.');
  const [updated] = await transaction
    .update(paymentPayouts)
    .set({
      ...providerFacts,
    ...
    .returning();
  if (!updated)
    throw new MoneyDomainError('PAYOUT_UPDATE_FAILED', 'Failed Payout state could not be saved.');
  await transaction.insert(paymentPayoutStatusHistory).values({
    payoutId: payout.id,
    fromStatus: payout.payoutStatus,
    toStatus: nextStatus,
...
const completeClaimedEvent = async (eventId: string) =>
  db.transaction(async (transaction) => {
    ...
    const source = event.eventType.startsWith('reconciliation.')
      ...
      : providerEventSource;
    await applyPayoutOutcomeInTransaction(
      transaction,
      {
        ...
        actualFeeSatang:
          event.providerActualFeeSatang === null ? null : satang(event.providerActualFeeSatang),
        actualTaxSatang:
          event.providerActualTaxSatang === null ? null : satang(event.providerActualTaxSatang),
        actualDebitSatang:
          event.providerActualDebitSatang === null
            ? null
            : positiveSatang(event.providerActualDebitSatang),
    ...
    await insertEventHistory(
      ...
      'PROCESSED',
      source,
      'Provider event applied to the Payout.'
    );
...
export const processPayoutProviderEvent = async (
  ...
  const [claimed] = await db.transaction((transaction) =>
    claimEvents(transaction, { eventId, now })
  );
  if (!claimed) {
...
export const retryPayoutProviderEvent = async (eventId: string): Promise<PayoutProviderEvent> =>
  db.transaction(async (transaction) => {
    const [event] = await transaction
      .select()
      .from(paymentProviderEventInbox)
...
export const reconcilePayout = async (
  ...
  if (payout.providerReference && outcome.providerReference !== payout.providerReference) {
    throw new ProviderEventError(
      'PROVIDER_EVENT_INVALID',
      'Provider reconciliation returned a different Payout reference.'
    );
  }
  ...
  const event = await db.transaction(async (transaction) => {
    ...
    if (created) {
      await insertEventHistory(
        ...
        'RECEIVED',
        reconciliationSource,
        'Provider state fetched for Payout reconciliation.'
      );
      return providerEventFromRecord(created);
    }
    const [existing] = await transaction
    ...
    if (!existing || existing.resourceType !== 'PAYOUT' || existing.payloadHash !== payloadHash) {
      throw new ProviderEventError(
        'PROVIDER_EVENT_CONFLICT',
        'Provider reconciliation outcome could not be retained.'
      );
    }
    return providerEventFromRecord(existing);
  });
