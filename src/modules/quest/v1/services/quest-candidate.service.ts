...
import type {
  ...
  TeamCreateInput,
  TeamUpdateInput,
} from '../schemas/quest-candidate.schema';
...
const applicationResult = (
  row: typeof applicationFields extends never
    ? never
    : {
        id: string;
        questId: string;
...
export const createApplication = async (
  ...
  db.transaction(async (tx) => {
    ...
    const [existing] = await tx
      ...
      .where(and(eq(questApplication.questId, questId), eq(questApplication.workerId, workerId)))
      .limit(1);
    if (existing) return { outcome: 'already-exists' };
    const [row] = await tx
      .insert(questApplication)
      .values({ questId, workerId, reworkLimit: data.reworkLimit ?? 0, appliedAt: now })
...
export const getApplication = async (userId: string, questId: string, applicationId: string) => {
  const [row] = await db
    ...
    .from(questApplication)
    .innerJoin(quest, eq(questApplication.questId, quest.id))
    .where(
      and(
        eq(questApplication.id, applicationId),
        eq(questApplication.questId, questId),
        eq(quest.apiVersion, questApiVersion.v1),
        eq(quest.mode, questMode.candidate),
        eq(quest.participation, questParticipation.solo),
        eq(quest.questStatus, questStatus.open),
        or(eq(questApplication.workerId, userId), eq(quest.hirerId, userId))
      )
    )
    .limit(1);
  return row;
...
export const updateApplication = async (
  ...
  db.transaction(async (tx) => {
    ...
    const [row] = await tx
      ...
      .where(
        and(
          eq(questApplication.id, applicationId),
          eq(questApplication.questId, questId),
          eq(questApplication.workerId, userId)
        )
      )
      .limit(1)
      .for('update');
    ...
    const [updated] = await tx
      ...
      .set({ reworkLimit: data.reworkLimit })
...
const memberRows = async (database: Database, teamId: string) =>
  database
...
const withMembers = async (database: Database, row: TeamRecord): Promise<TeamWithMembers> => ({
  ...row,
  members: await memberRows(database, row.id),
});
...
const ensureNoMembership = async (tx: Tx, questId: string, userId: string) => {
  const [row] = await tx
    .select({ id: questTeamMember.teamId })
    .from(questTeamMember)
    .innerJoin(questTeam, eq(questTeamMember.teamId, questTeam.id))
    .where(and(eq(questTeam.questId, questId), eq(questTeamMember.userId, userId)))
    .limit(1);
  return !row;
};

const teamMemberAccess = (userId: string) =>
  exists(sql`(
  select 1
  ...
  db.transaction(async (tx) => {
    ...
    const [team] = await tx
      ...
      .values({
        ...
        leaderId,
        name: data.name,
        reworkLimit: data.reworkLimit ?? 0,
        createdAt: now,
      })
...
const selectTeamForUser = async (
  ...
  const query = database
    .select(teamFields)
    .from(questTeam)
    .innerJoin(quest, eq(questTeam.questId, quest.id))
    .where(
      and(
        eq(questTeam.id, teamId),
        eq(questTeam.questId, questId),
        eq(quest.apiVersion, questApiVersion.v1),
        eq(quest.mode, questMode.candidate),
        eq(quest.participation, questParticipation.group),
        eq(quest.questStatus, questStatus.open),
        sql`${questTeam.teamStatus} <> ${teamStatus.disbanded}`,
        teamAccess(userId)
      )
    )
    .limit(1);
...
export const listTeams = async (userId: string, questId: string): Promise<TeamCollection> =>
  db.transaction(async (tx) => {
    const current = await lockQuest(tx, questId);
    if (
      !current ||
    ...
    const rows = await tx
      ...
      .orderBy(asc(questTeam.createdAt), asc(questTeam.id));
...
export const updateTeam = async (
  ...
  db.transaction(async (tx) => {
    ...
    if (data.name === undefined && data.reworkLimit === undefined)
      return { outcome: 'not-editable' };
    const [updated] = await tx
      .update(questTeam)
      .set({
        ...(data.name !== undefined ? { name: data.name } : {}),
        ...(data.reworkLimit !== undefined ? { reworkLimit: data.reworkLimit } : {}),
      })
...
const lockTeamMembers = async (tx: Tx, teamId: string) =>
  tx
    .select({ userId: questTeamMember.userId, joinedAt: questTeamMember.joinedAt })
    .from(questTeamMember)
    .where(eq(questTeamMember.teamId, teamId))
...
const revokePendingTeamInvitations = async (tx: Tx, teamId: string, now: Date) => {
  await tx
    ...
    .where(
      and(
        eq(questTeamInvitation.teamId, teamId),
...
export const leaveTeam = async (
  ...
  db.transaction(async (tx) => {
    ...
    const [row] = await tx
      .select(teamFields)
      .from(questTeam)
      .where(and(eq(questTeam.id, teamId), eq(questTeam.questId, questId)))
      .limit(1)
      .for('update');
    ...
    await tx
      .delete(questTeamMember)
      .where(and(eq(questTeamMember.teamId, teamId), eq(questTeamMember.userId, userId)));
    const remaining = members.filter((member) => member.userId !== userId);
    if (remaining.length === 0) {
      await revokePendingTeamInvitations(tx, teamId, now);
      const [updated] = await tx
        .update(questTeam)
        .set({ teamStatus: teamStatus.disbanded })
        .where(eq(questTeam.id, teamId))
        .returning(teamFields);
      return withMembers(tx, updated);
    }
...
export const removeTeamMember = async (
  ...
  db.transaction(async (tx) => {
    ...
    await tx
      .delete(questTeamMember)
      .where(and(eq(questTeamMember.teamId, teamId), eq(questTeamMember.userId, memberId)));
...
export const submitTeam = async (
  ...
  db.transaction(async (tx) => {
    ...
    const members = await tx
      .select({ userId: questTeamMember.userId })
      .from(questTeamMember)
      .where(eq(questTeamMember.teamId, teamId));
    ...
    const [updated] = await tx
      ...
      .where(and(eq(questTeam.id, teamId), eq(questTeam.teamStatus, teamStatus.forming)))
      .returning(teamFields);
    return withMembers(tx, updated);
  });
...
const selectInvitation = async (
  ...
  const query = database
    ...
    .innerJoin(questTeam, eq(questTeamInvitation.teamId, questTeam.id))
    .where(
      and(
...
export const createInvitation = async (
  ...
  db.transaction(async (tx) => {
    ...
    if (
      ...
      team.teamStatus !== teamStatus.forming ||
      data.invitedUserId === leaderId ||
      data.invitedUserId === current.hirerId
    )
      return { outcome: 'not-eligible' };
    const [pending] = await tx
      ...
      .where(
        and(
          eq(questTeamInvitation.teamId, teamId),
          eq(questTeamInvitation.invitedUserId, data.invitedUserId),
          eq(questTeamInvitation.invitationStatus, 'INVITATION_PENDING')
        )
      )
      .limit(1);
    ...
    const [invitedUser] = await tx
      ...
      .from(authUser)
      .where(eq(authUser.id, data.invitedUserId))
      .limit(1);
    if (!invitedUser) return { outcome: 'not-eligible' };
    if (!(await ensureNoMembership(tx, questId, data.invitedUserId)))
      return { outcome: 'already-member' };
    const [created] = await tx
      .insert(questTeamInvitation)
      .values({
        teamId,
...
export const listTeamInvitations = async (leaderId: string, questId: string, teamId: string) => {
  ...
  return db
    ...
    .orderBy(asc(questTeamInvitation.createdAt), asc(questTeamInvitation.id));
};

export const listOwnInvitations = async (userId: string) =>
  db
    .select(invitationFields)
    .from(questTeamInvitation)
    .innerJoin(questTeam, eq(questTeamInvitation.teamId, questTeam.id))
    ...
    .where(
      and(
        eq(questTeamInvitation.invitedUserId, userId),
        eq(quest.apiVersion, questApiVersion.v1),
        eq(quest.mode, questMode.candidate),
        eq(quest.participation, questParticipation.group),
        eq(quest.questStatus, questStatus.open),
...
export const revokeInvitation = async (
  ...
  db.transaction(async (tx) => {
    ...
    const [updated] = await tx
      .update(questTeamInvitation)
      .set({ invitationStatus: 'INVITATION_REVOKED', respondedAt: now })
      .where(eq(questTeamInvitation.id, invitationId))
      .returning(invitationFields);
...
const respondToInvitation = async (
  userId: string,
  invitationId: string,
  response: 'INVITATION_ACCEPTED' | 'INVITATION_DECLINED',
  now: Date
): Promise<InvitationOutcome> =>
  db.transaction(async (tx) => {
    const [identity] = await tx
      .select({ questId: questTeam.questId, teamId: questTeam.id })
      .from(questTeamInvitation)
      .innerJoin(questTeam, eq(questTeamInvitation.teamId, questTeam.id))
      .where(
        and(eq(questTeamInvitation.id, invitationId), eq(questTeamInvitation.invitedUserId, userId))
      )
      .limit(1);
    ...
    const team = await tx
      .select(teamFields)
      .from(questTeam)
      .where(eq(questTeam.id, identity.teamId))
      .limit(1)
      .for('update');
    ...
    if (response === 'INVITATION_DECLINED') {
      const [updated] = await tx
        .update(questTeamInvitation)
        .set({ invitationStatus: response, respondedAt: now })
        .where(eq(questTeamInvitation.id, invitationId))
        .returning(invitationFields);
      return invitationResult(updated);
    }
    ...
    if (!(await ensureNoMembership(tx, identity.questId, userId)))
      return { outcome: 'already-member' };
...
export const acceptInvitation = (userId: string, invitationId: string, now = new Date()) =>
  respondToInvitation(userId, invitationId, 'INVITATION_ACCEPTED', now);
export const declineInvitation = (userId: string, invitationId: string, now = new Date()) =>
  respondToInvitation(userId, invitationId, 'INVITATION_DECLINED', now);
export const getOwnInvitation = async (userId: string, invitationId: string) =>
  db
    .select(invitationFields)
    .from(questTeamInvitation)
    .innerJoin(questTeam, eq(questTeamInvitation.teamId, questTeam.id))
...
export type CandidateSelectionTarget =
  { type: 'APPLICATION'; id: string } | { type: 'TEAM'; id: string };
...
const hashSelectionRequest = async (
  hirerId: string,
  questId: string,
...
const replaySelection = async (
  tx: QuestTransaction,
  command: SelectionCommandRecord,
  hirerId: string,
  questId: string,
  target: CandidateSelectionTarget,
  requestHash: string
  ...
  if (
    ...
    !Array.isArray(command.resultAssignmentIds) ||
    command.resultQuestStatus !== questStatus.assigned
  )
    return { outcome: 'idempotency-unavailable' };
  const ids = command.resultAssignmentIds.filter((id): id is string => typeof id === 'string');
  const rows = await tx
    .select(selectionAssignmentFields)
    .from(questAssignment)
    .where(inArray(questAssignment.id, ids));
  const byId = new Map(rows.map((row) => [row.id, row]));
  const assignments = ids
    .map((id) => byId.get(id))
    .filter((row): row is SelectionAssignment => row !== undefined);
...
const acquireSelectionCommand = async (
  ...
  const [existing] = await tx
    .select(selectionCommandFields)
    .from(questCandidateSelectionCommand)
    .where(eq(questCandidateSelectionCommand.commandId, commandId))
    .limit(1)
    .for('update');
  if (existing) return replaySelection(tx, existing, hirerId, questId, target, requestHash);
  const [created] = await tx
    .insert(questCandidateSelectionCommand)
    .values({
      commandId,
      hirerId,
      questId,
    ...
    .onConflictDoNothing({ target: questCandidateSelectionCommand.commandId })
    .returning(selectionCommandFields);
  ...
  const [concurrent] = await tx
    ...
    .for('update');
  return concurrent
    ? replaySelection(tx, concurrent, hirerId, questId, target, requestHash)
...
const selectionTransition = (
  ...
  })) as [AcceptedWorker, ...AcceptedWorker[]];
  return {
...
export const selectCandidate = async (
  ...
  return db.transaction(async (tx) => {
    ...
    const command = await acquireSelectionCommand(
      tx,
      commandId,
      hirerId,
      questId,
    ...
    const discardCommand = async (outcome: SelectionOutcomeCode) => {
      await tx
        .delete(questCandidateSelectionCommand)
        .where(eq(questCandidateSelectionCommand.commandId, commandId));
    ...
    if (
      current.mode !== questMode.candidate ||
      (target.type === 'APPLICATION' && current.participation !== questParticipation.solo) ||
      (target.type === 'TEAM' && current.participation !== questParticipation.group)
    )
      return discardCommand('not-allowed');
    ...
    if (target.type === 'APPLICATION') {
      ...
      if (selected.applicationStatus !== applicationStatus.applied || selected.workerId === hirerId)
        return discardCommand('not-selectable');
      ...
      await tx
        ...
        .where(eq(questApplication.id, selected.id));
      await tx
        .update(questApplication)
        .set({ applicationStatus: applicationStatus.rejected })
        .where(
          and(
            eq(questApplication.questId, questId),
            eq(questApplication.applicationStatus, applicationStatus.applied),
            sql`${questApplication.id} <> ${selected.id}`
          )
        );
    } else {
      ...
      const members = await tx
        .select({ userId: questTeamMember.userId, joinedAt: questTeamMember.joinedAt })
        .from(questTeamMember)
        .where(eq(questTeamMember.teamId, selected.id))
        .orderBy(asc(questTeamMember.joinedAt), asc(questTeamMember.userId))
        .for('update');
      ...
      if (
        ...
        roster.includes(hirerId) ||
        roster.some((workerId) =>
          assignmentRows.some((assignment) => assignment.workerId === workerId)
        )
      )
        return discardCommand('already-assigned');
      await tx
        ...
        .where(eq(questTeam.id, selected.id));
      await tx
        .update(questTeam)
        .set({ teamStatus: teamStatus.rejected })
        .where(
          and(
            eq(questTeam.questId, questId),
            eq(questTeam.teamStatus, teamStatus.submitted),
            sql`${questTeam.id} <> ${selected.id}`
          )
        );
    }
    ...
    await tx
      ...
      .set({
        resultAssignmentIds: assignments.map((assignment) => assignment.id),
        resultQuestStatus: questStatus.assigned,
