...
const ownerConfirmationCondition = (owner: Owner) =>
  owner.workerId
...
const findOwner = async (
  ...
  const [current] = await tx
    .select({ mode: quest.mode, participation: quest.participation })
    .from(quest)
    .where(eq(quest.id, questId))
    .limit(1);
    ...
    const teams = tx
      ...
      .where(
        and(
          ...
          eq(questTeamMember.userId, userId),
    ...
    const [application] = await tx
      ...
      .where(
        and(
          ...
          eq(questApplication.applicationStatus, 'APPLICATION_SELECTED')
        )
      )
      .limit(1)
      .for('update');
...
const fileIdsForProof = async (tx: Tx, userId: string, fileIds: string[]) => {
  ...
  const rows = await tx
    ...
    .from(file)
    .where(
      and(inArray(file.id, fileIds), eq(file.uploadedByUserId, userId), isNull(file.deletedAt))
    )
    .for('update');
...
const loadProof = async (tx: Tx, id: string): Promise<QuestProofRow | undefined> => {
  const [row] = await tx
    ...
    .where(eq(proofSubmission.id, id))
    .limit(1);
  if (!row) return undefined;
  const images = await tx
    .select({ fileId: proofSubmissionImage.fileId })
    .from(proofSubmissionImage)
    .where(eq(proofSubmissionImage.proofSubmissionId, id))
  ...
  const [row] = await tx
    ...
    .where(
      and(
        ...
        owner.teamId
          ? sql`${proofSubmission.workerId} IS NOT NULL`
          : sql`${proofSubmission.teamId} IS NOT NULL`
      )
    )
    .limit(1);
  return Boolean(row);
...
const ownerHasSubmission = async (tx: Tx, questId: string, owner: Owner, statuses: string[]) => {
  const rows = await tx
    .select({ id: proofSubmission.id, submissionStatus: proofSubmission.submissionStatus })
    .from(proofSubmission)
    .where(
      and(
        eq(proofSubmission.questId, questId),
        ownerCondition(owner),
        inArray(proofSubmission.submissionStatus, statuses)
      )
    );
...
const sameOwner = (left: Owner, right: Owner) =>
  left.workerId === right.workerId && left.teamId === right.teamId;
const isReviewLifecycle = (status: string) =>
  status === questStatus.submitted ||
  status === questStatus.rework ||
  status === questStatus.failed;

const latestRejectedOwner = async (tx: Tx, questId: string): Promise<Owner | undefined> => {
  const [row] = await tx
    .select({ workerId: proofSubmission.workerId, teamId: proofSubmission.teamId })
    .from(proofSubmission)
    .where(
      and(
        eq(proofSubmission.questId, questId),
        inArray(proofSubmission.submissionStatus, [...notApprovedProofStatuses])
      )
    )
    .orderBy(sql`${proofSubmission.reviewedAt} DESC NULLS LAST`, sql`${proofSubmission.id} DESC`)
    .limit(1);
  return row?.workerId
...
const ownersForQuest = async (
  tx: Tx,
  current: { id: string; mode: string; participation: string; questStatus?: string }
): Promise<Owner[]> => {
  if (current.mode === questMode.candidate && current.participation === questParticipation.group) {
    const [team] = await tx
      .select({ id: questTeam.id })
      .from(questTeam)
      .where(and(eq(questTeam.questId, current.id), eq(questTeam.teamStatus, teamStatus.selected)))
      .limit(1)
      .for('update');
  ...
  const assignments = await tx
    ...
    .where(
      and(
        ...
        eq(questAssignment.assignmentStatus, assignmentState)
      )
    )
    .for('update');
...
const allObligationsSubmitted = async (
  tx: Tx,
  ...
  if (!current.proofRequired) {
    const confirmations = await tx
      .select({
        workerId: questCompletionConfirmation.workerId,
        teamId: questCompletionConfirmation.teamId,
      })
      .from(questCompletionConfirmation)
      .where(eq(questCompletionConfirmation.questId, current.id));
    return owners.every((owner) =>
      confirmations.some(
        (confirmation) =>
          owner.workerId === confirmation.workerId && owner.teamId === confirmation.teamId
      )
    );
  }
  return Promise.all(
    owners.map((owner) =>
      ownerHasSubmission(tx, current.id, owner, ['PROOF_PENDING', ...approvedProofStatuses])
    )
  ).then((values) => values.every(Boolean));
};

const allObligationsApproved = async (
  tx: Tx,
  current: { id: string; mode: string; participation: string; proofRequired: boolean }
) => {
  ...
  if (owners.length === 0) return false;
  return Promise.all(
    owners.map((owner) => ownerHasSubmission(tx, current.id, owner, [...approvedProofStatuses]))
  ).then((values) => values.every(Boolean));
};

const moveIfSubmitted = async (
  tx: Tx,
  current: {
    id: string;
    questStatus: string;
...
const insertProof = async (
  ...
  if (input.fileIds.length > 0)
    await tx.insert(proofSubmissionImage).values(
...
export const submitProof = async (
  ...
  db.transaction(async (tx) => {
    ...
    if (
      ...
      current.questStatus !== questStatus.rework
    )
      return { outcome: 'invalid-state' };
    if (current.dueAt && now.getTime() >= current.dueAt.getTime())
      return { outcome: 'invalid-state' };
    ...
    if (storedImages.length > 0) {
      ...
      const createdFiles = await tx
        .insert(file)
        .values(storedImages.map((image) => ({ ...image, uploadedByUserId: userId })))
    ...
    if (await hasOppositeOwnerSubmission(tx, questId, owner))
      return { outcome: 'already-submitted' };
    if (await ownerHasSubmission(tx, questId, owner, ['PROOF_PENDING', ...approvedProofStatuses]))
      return { outcome: 'already-submitted' };
    if (current.mode === questMode.noCandidate && rejected > 0) return { outcome: 'no-rework' };
    const proof = await insertProof(tx, {
      questId,
      userId,
      owner,
      content,
...
export const confirmProofFreeWork = async (
  userId: string,
  questId: string,
  now = new Date()
): Promise<{ outcome: string } | { confirmed: boolean; questStatus: string }> =>
  db.transaction(async (tx) => {
    const current = await lockQuest(tx, questId);
    if (!current) return { outcome: 'not-found' };
    if (current.proofRequired) return { outcome: 'proof-required' };
    ...
    const [existing] = await tx
      ...
      .from(questCompletionConfirmation)
      .where(
        and(eq(questCompletionConfirmation.questId, questId), ownerConfirmationCondition(owner))
      )
      .limit(1)
      .for('update');
    if (!existing)
      await tx.insert(questCompletionConfirmation).values({
        questId,
...
export const reviewProof = async (
  ...
  db.transaction(async (tx) => {
    ...
    if (
      current.questStatus === questStatus.failed &&
      ((current.dueAt && proof.submittedAt >= current.dueAt) ||
        !current.failedAt ||
        proof.submittedAt >= current.failedAt)
    ) {
    ...
    const validOwners =
      proofOwner &&
      (await ownersForQuest(tx, current)).some((owner) => sameOwner(owner, proofOwner));
    if (!validOwners) return { outcome: 'not-found' };
    const status = decision;
    await tx
      .update(proofSubmission)
      .set({ submissionStatus: status, reviewNote: note, reviewedAt: now })
    ...
    if (decision === 'PROOF_NOT_APPROVED') {
      ...
      if (proof.teamId) {
        const [team] = await tx
          .select({ reworkLimit: questTeam.reworkLimit })
          .from(questTeam)
          .where(eq(questTeam.id, proof.teamId))
          .limit(1)
          .for('update');
        limit = team?.reworkLimit ?? 0;
      } else {
        const [application] = await tx
          .select({ reworkLimit: questApplication.reworkLimit })
          .from(questApplication)
          .where(
            and(
              eq(questApplication.questId, questId),
              eq(questApplication.workerId, proof.workerId!)
            )
          )
          .limit(1)
          .for('update');
    ...
    return {
      proof: updated!,
      questStatus: (
        await tx.select({ questStatus: quest.questStatus }).from(quest).where(eq(quest.id, questId))
      )[0]!.questStatus,
    };
  });
...
export const listProofs = async (userId: string, questId: string) => {
  const rows = await db
    ...
    .where(
      and(
        eq(proofSubmission.questId, questId),
        or(eq(quest.hirerId, userId), eq(proofSubmission.submittedByUserId, userId))
      )
    )
    ...
    rows.map(async (row) => ({
      ...
      images: (
        await db
          .select({ fileId: proofSubmissionImage.fileId })
          .from(proofSubmissionImage)
          .where(eq(proofSubmissionImage.proofSubmissionId, row.id))
          .orderBy(asc(proofSubmissionImage.position))
      ).map(({ fileId }) => fileId),
    }))
  );
...
const autoApproveDueProofFreeQuests = async (tx: Tx, now: Date) => {
  const candidates = await tx
    .select({ id: quest.id, hirerId: quest.hirerId })
    .from(quest)
    .where(
      and(
...
export const autoApproveDueProofs = async (now = new Date()): Promise<string[]> =>
  db.transaction(async (tx) => {
    const candidates = await tx
      .select({
        id: proofSubmission.id,
    ...
    for (const candidate of candidates) {
      ...
      if (
        current.questStatus === questStatus.failed &&
        (!current.failedAt || proof.submittedAt >= current.failedAt)
      )
        continue;
      await tx
        .update(proofSubmission)
        .set({ submissionStatus: 'PROOF_APPROVED', reviewedAt: now })
        .where(
          and(
            eq(proofSubmission.id, candidate.id),
            eq(proofSubmission.submissionStatus, 'PROOF_PENDING')
      ...
      if (current.questStatus === questStatus.failed) {
        await settleApprovedLegacyQuestProofAfterFailureInTransaction(
          tx,
          candidate.questId,
          candidate.id
        );
...
export const getRequiredConfirmationCount = async (questId: string) =>
  db.transaction(async (tx) => {
    const current = await lockQuest(tx, questId);
