...
const detectContentType = async (
  ...
  if (!detectedContentType || declaredContentType !== detectedContentType) {
    throw new UnsupportedWorkChatAttachmentError(
...
export const createWorkChatStorage = ({
  ...
  const upload = async (
    memberId: string,
    file: File
  ): Promise<StoredWorkChatAttachment & { fileName: string }> => {
    const bytes = new Uint8Array(await file.arrayBuffer());
    if (bytes.length === 0)
      throw new UnsupportedWorkChatAttachmentError('Attachment file is empty');
  ...
  const remove = async (
    stored: Pick<StoredWorkChatAttachment, 'bucket' | 'objectKey'>
  ): Promise<void> => {
  ...
  const linkFor = ({
    bucket,
    objectKey,
  }: Pick<StoredWorkChatAttachment, 'bucket' | 'objectKey'>): WorkChatAttachmentLink => {
