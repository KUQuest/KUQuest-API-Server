...
interface PolicyRow {
  id: string; minimum_top_up: string; maximum_top_up: string;
  minimum_payout: string; maximum_payout: string; quote_seconds: string;
  topup_fee: string; topup_tax_bps: string; payout_fee: string; payout_tax_bps: string;
}

interface TopUpRow {
  id: string; internal_reference: string; credit_baht: string; payment_total_baht: string;
  status: string; qr_payload: string | null; qr_expires_at: Date | string | null;
  provider_reference: string | null; created_at: Date | string; updated_at: Date | string;
}

interface AccountRow {
  id: string; given_name: string; surname: string; account_holder_name: string;
  account_number: string; bank_code: string; masked_last_four: string; created_at: Date | string;
}

interface PayoutRow {
  id: string; provider_reference: string | null; principal_baht: string;
  maximum_debit_baht: string; status: string; destination_bank_code: string;
  destination_account_number: string; created_at: Date | string; updated_at: Date | string;
}
...
const topUp = (row: TopUpRow): TopUp => ({
  ...
  currency: 'THB', status: row.status, qr_string: row.qr_payload,
...
const account = (row: AccountRow): PayoutAccount => ({
  id: row.id, given_name: row.given_name, surname: row.surname,
  account_holder_name: row.account_holder_name, bank_code: row.bank_code,
  masked_account_number: `••••${row.masked_last_four}`, created_at: iso(row.created_at),
});

const payout = (row: PayoutRow): Payout => ({
  id: row.id, reference: `payout:${row.id}`,
  ...
  currency: 'THB', status: row.status,
  destination: {
...
export class PostgresPaymentsRepository implements PaymentsRepository {
  constructor(private readonly database: Sql, private readonly xendit: XenditClient) {}
  ...
  async createTopUpQuote(userId: string, creditBaht: number): Promise<TopUpQuote> {
    ...
    if (!Number.isSafeInteger(creditBaht) || creditBaht < minimum || creditBaht > maximum) {
      throw new MoneyError(422, 'VALIDATION_FAILED', `Amount must be between ${minimum} and ${maximum} THB.`);
    }
    const feeSatang = integer(policy.topup_fee, 'topup_fee');
    const taxSatang = Math.ceil(feeSatang * integer(policy.topup_tax_bps, 'topup_tax_bps') / 10_000);
    const feeBaht = Math.ceil(feeSatang / 100);
    const taxBaht = Math.ceil(taxSatang / 100);
    const total = creditBaht + feeBaht + taxBaht;
    const id = crypto.randomUUID();
    const expiresAt = new Date(Date.now() + integer(policy.quote_seconds, 'quote_seconds') * 1000).toISOString();
    await this.database`
    ...
    `;
    return { id, credit_baht: creditBaht, fee_baht: feeBaht, tax_baht: taxBaht,
      payment_total_baht: total, currency: 'THB', expires_at: expiresAt };
  }
  ...
  async createTopUp(userId: string, quoteId: string, idempotencyKey: string): Promise<TopUp> {
    ...
    const prepared = await this.database.begin(async (transaction) => {
      ...
      return { id, reference, amount: integer(quote.payment_total_baht as string, 'payment_total'), idempotencyId };
    });
    if ('replay' in prepared) return prepared.replay!;
    const expiresAt = new Date(Date.now() + 30 * 60_000).toISOString();
    try {
      const provider = await this.xendit.createPromptPay({
        reference: prepared.reference, amountBaht: prepared.amount, expiresAt,
      });
      ...
      const next=error instanceof MoneyError&&error.status===422?'FAILED':'AWAITING_RECONCILIATION';
  ...
  private async getTopUpIn(database: Database, userId: string, topUpId: string): Promise<TopUp> {
    const [row] = (await database`SELECT id::text,internal_reference,credit_baht::text,
    ...
    if (!row) throw new MoneyError(404, 'NOT_FOUND', 'The top-up was not found.');
    return topUp(row);
  }
  getTopUp(userId: string, topUpId: string) { return this.getTopUpIn(this.database, userId, topUpId); }
  async listTopUps(userId: string): Promise<TopUp[]> {
    const rows = (await this.database`SELECT id::text,internal_reference,credit_baht::text,
      payment_total_baht::text,status,qr_payload,qr_expires_at,provider_reference,created_at,updated_at
  ...
  async savePayoutAccount(userId: string, input: PayoutAccountInput): Promise<PayoutAccount> {
    if (!input.account_number.trim() || !input.bank_code.trim()) {
      throw new MoneyError(422, 'VALIDATION_FAILED', 'Bank code and account number are required.');
    }
  ...
  private async accountRow(userId: string, database: Database = this.database): Promise<AccountRow | null> {
    const [row] = (await database`SELECT id::text,given_name,surname,account_holder_name,account_number,
  ...
  async getPayoutAccount(userId: string): Promise<PayoutAccount | null> {
    const row = await this.accountRow(userId); return row ? account(row) : null;
  }

  async createPayoutQuote(userId: string, receiptBaht: number): Promise<PayoutQuote> {
    const policy = await this.policy();
    const minimum = integer(policy.minimum_payout, 'minimum_payout');
    const maximum = integer(policy.maximum_payout, 'maximum_payout');
    if (!Number.isSafeInteger(receiptBaht) || receiptBaht < minimum || receiptBaht > maximum) {
      throw new MoneyError(422, 'VALIDATION_FAILED', `Amount must be between ${minimum} and ${maximum} THB.`);
    }
    const payoutAccount = await this.accountRow(userId);
    if (!payoutAccount) throw new MoneyError(422, 'VALIDATION_FAILED', 'Save a payout account first.');
    const feeSatang = integer(policy.payout_fee, 'payout_fee');
    const taxSatang = Math.ceil(feeSatang * integer(policy.payout_tax_bps, 'payout_tax_bps') / 10_000);
    const feeBaht = Math.ceil(feeSatang / 100); const taxBaht = Math.ceil(taxSatang / 100);
    const maximumDebit = receiptBaht + feeBaht + taxBaht;
    const id = crypto.randomUUID();
    const expiresAt = new Date(Date.now() + integer(policy.quote_seconds, 'quote_seconds') * 1000).toISOString();
    await this.database`INSERT INTO payout_quotes(id,user_id,payout_account_id,policy_revision_id,
    ...
    return { id,payout_account_id:payoutAccount.id,receipt_baht:receiptBaht,
      maximum_fee_baht:feeBaht,maximum_tax_baht:taxBaht,maximum_debit_baht:maximumDebit,
      currency:'THB',expires_at:expiresAt };
  }
  ...
  async createPayout(userId: string, quoteId: string, idempotencyKey: string): Promise<Payout> {
    const prepared = await this.database.begin(async (transaction) => {
      ...
      const [wallet]=(await transaction`SELECT id::text,status,earnings_balance_baht::text AS earnings
        FROM wallets WHERE user_id=${userId} FOR UPDATE`) as unknown as Array<{id:string;status:string;earnings:string}>;
      if(!wallet) throw new Error('Wallet is not provisioned.');
      if(wallet.status!=='ACTIVE') throw new MoneyError(423,'WALLET_FROZEN','The wallet is frozen.');
      const debit=integer(quote.maximum_debit_baht as string,'maximum_debit');
      if(integer(wallet.earnings,'earnings')<debit) throw new MoneyError(422,'INSUFFICIENT_EARNINGS_BALANCE','The wallet does not have enough available earnings.');
      const accounts=(await transaction`SELECT id::text,type FROM ledger_accounts WHERE wallet_id=${wallet.id}
    ...
    if('replay' in prepared) return prepared.replay!;
    try {
      const provider=await this.xendit.createPayout({reference:`kuquest-payout-${prepared.id}`,
        amountSatang:prepared.principal*100,account:prepared.account});
      return this.database.begin(async transaction=>{
      ...
      if(error instanceof MoneyError&&error.status===422)await this.releaseRejectedPayout(userId,prepared.id,prepared.idemId,prepared.debit);
      else await this.database`UPDATE payouts SET status='AWAITING_RECONCILIATION',updated_at=now() WHERE id=${prepared.id}`;
      throw error;
    }
  }

  private async releaseRejectedPayout(userId:string,payoutId:string,idempotencyId:string,amount:number):Promise<void>{
    await this.database.begin(async transaction=>{
      const [wallet]=(await transaction`SELECT id::text FROM wallets WHERE user_id=${userId} FOR UPDATE`) as unknown as Array<{id:string}>;
      const accounts=(await transaction`SELECT id::text,type FROM ledger_accounts WHERE wallet_id=${wallet?.id}
        AND type IN ('EARNINGS','PAYOUT_RESERVED')`) as unknown as Array<{id:string;type:string}>;
      const earnings=accounts.find(item=>item.type==='EARNINGS')?.id;
      const reserved=accounts.find(item=>item.type==='PAYOUT_RESERVED')?.id;
      if(!earnings||!reserved)throw new Error('Payout release accounts are not provisioned.');
  ...
  private async getPayoutIn(database:Database,userId:string,payoutId:string):Promise<Payout>{
    const [row]=(await database`SELECT id::text,provider_reference,principal_baht::text,maximum_debit_baht::text,
    ...
    if(!row) throw new MoneyError(404,'NOT_FOUND','The payout was not found.'); return payout(row);
  }
  getPayout(userId:string,payoutId:string){return this.getPayoutIn(this.database,userId,payoutId);}
  async listPayouts(userId:string):Promise<Payout[]>{
    const rows=(await this.database`SELECT id::text,provider_reference,principal_baht::text,maximum_debit_baht::text,
      status,destination_bank_code,destination_account_number,created_at,updated_at FROM payouts
      WHERE user_id=${userId} ORDER BY created_at DESC LIMIT 50`) as unknown as PayoutRow[]; return rows.map(payout);
  }

  async processStoredWebhooks(limit=20):Promise<number>{
    const events=(await this.database`SELECT id::text,payload FROM provider_webhook_events
      WHERE status IN ('RECEIVED','RETRYABLE') AND attempts < 5
      ORDER BY received_at LIMIT ${limit}`) as unknown as Array<{id:string;payload:unknown}>;
    let processed=0;
    for(const event of events){
      try { await this.processWebhook(event.id,event.payload); processed++; }
      catch(error){
        const message=error instanceof Error?error.message:'Unknown processing failure';
        await this.database.begin(async transaction=>{
          const [current]=(await transaction`SELECT status,attempts FROM provider_webhook_events WHERE id=${event.id} FOR UPDATE`) as unknown as Array<{status:string;attempts:number}>;
          if(!current)return;
          const next=current.attempts+1>=5?'DEAD_LETTER':'RETRYABLE';
  ...
  private async processWebhook(eventId:string,payload:unknown):Promise<void>{
    await this.database.begin(async transaction=>{
      const [claimed]=(await transaction`UPDATE provider_webhook_events SET status='PROCESSING',claimed_at=now(),attempts=attempts+1
        WHERE id=${eventId} AND status IN ('RECEIVED','RETRYABLE') RETURNING id`) as unknown as Array<{id:string}>;
      if(!claimed)return;
      const root=object(payload); const data=object(root?.data)??root; if(!data) throw new Error('Webhook data is missing.');
      const eventType=(string(root?.event)??'unknown').toLowerCase();
      const status=(string(data.status)??string(root?.status)??'').toUpperCase();
      const providerReference=string(data.payment_request_id)??string(data.payout_id)??string(data.id);
      const reference=string(data.reference_id);
      if(!providerReference&&!reference) throw new Error('Webhook reference is missing.');
      const [topup]=(await transaction`SELECT id::text,user_id,internal_reference,provider_reference,credit_baht::text,payment_total_baht::text,currency,status,credited_ledger_transaction_id
        FROM top_ups WHERE provider_reference=${providerReference} OR internal_reference=${reference} FOR UPDATE`) as unknown as Array<{id:string;user_id:string;credit_baht:string;status:string;credited_ledger_transaction_id:string|null}>;
      if(topup){
        const identified=topup as typeof topup&{internal_reference:string;provider_reference:string|null};
        if(!eventType.includes('payment')||eventType.includes('payout'))throw new Error('Only payment events can update a top-up.');
        if(!reference||reference!==identified.internal_reference)throw new Error('Webhook reference does not match the top-up.');
        if(!providerReference||!identified.provider_reference||providerReference!==identified.provider_reference)
          throw new Error('Webhook provider identifier does not match the top-up.');
        const receivedAmount=typeof data.request_amount==='number'?data.request_amount:typeof data.amount==='number'?data.amount:null;
        if(receivedAmount===null||receivedAmount!==integer((topup as typeof topup&{payment_total_baht:string}).payment_total_baht,'payment_total'))
          throw new Error('Webhook amount does not match the top-up.');
        if(data.currency!=='THB')throw new Error('Webhook currency does not match the top-up.');
        await this.applyTopUpWebhook(transaction,topup,status);
      }
      else {
        const [out]=(await transaction`SELECT payout.id::text,payout.user_id,payout.provider_reference,
          ...
          WHERE payout.provider_reference=${providerReference} OR ${reference}=${`kuquest-payout-`}||payout.id::text
          FOR UPDATE OF payout`) as unknown as Array<{id:string;user_id:string;principal_baht:string;
            maximum_debit_baht:string;quoted_fee_satang:string;quoted_tax_satang:string;
            actual_debit_satang:string|null;status:string;final_ledger_transaction_id:string|null}>;
        if(!out) throw new Error('Webhook does not match a KUQuest operation.');
        const identified=out as typeof out&{provider_reference:string|null};
        if(!eventType.includes('payout'))throw new Error('Only payout events can update a payout.');
        if(!reference||reference!==`kuquest-payout-${out.id}`)throw new Error('Webhook reference does not match the payout.');
        if(!providerReference||!identified.provider_reference||providerReference!==identified.provider_reference)
          throw new Error('Webhook provider identifier does not match the payout.');
        const expectedPrincipal=integer((out as typeof out&{principal_baht:string}).principal_baht,'principal_baht');
        const receivedMajor=typeof data.amount==='number'?data.amount:null;
        const receivedMinor=typeof data.source_amount==='number'?data.source_amount:null;
        if(receivedMajor===null&&receivedMinor===null)throw new Error('Webhook payout amount is missing.');
        if((receivedMajor!==null&&receivedMajor!==expectedPrincipal)||(receivedMinor!==null&&receivedMinor!==expectedPrincipal*100))
          throw new Error('Webhook amount does not match the payout.');
        const currency=typeof data.currency==='string'?data.currency:typeof data.source_currency==='string'?data.source_currency:null;
        if(currency!=='THB')throw new Error('Webhook currency does not match the payout.');
        if(eventType.startsWith('v3_payout')){
          if(data.destination_currency!=='THB'||data.destination_amount!==expectedPrincipal*100)
            throw new Error('Webhook destination amount or currency does not match the payout.');
        }
        await this.applyPayoutWebhook(transaction,out,status);
      }
  ...
  private async applyTopUpWebhook(transaction:TransactionSql<Record<string,never>>,row:{id:string;user_id:string;credit_baht:string;status:string;credited_ledger_transaction_id:string|null},status:string){
    const failed=status==='FAILED'||status==='EXPIRED'||status==='CANCELLED';
    if(failed){
      if(row.credited_ledger_transaction_id||row.status==='SUCCEEDED'||row.status===status)return;
    ...
    if(status!=='SUCCEEDED'&&status!=='COMPLETED'&&status!=='PAID') return;
    if(row.credited_ledger_transaction_id)return;
    const [wallet]=(await transaction`SELECT id::text FROM wallets WHERE user_id=${row.user_id}`) as unknown as Array<{id:string}>;
    const accounts=(await transaction`SELECT id::text,type FROM ledger_accounts WHERE
      (wallet_id=${wallet?.id} AND type='SPENDING') OR code='SYSTEM:PROVIDER_ASSET'`) as unknown as Array<{id:string;type:string}>;
    const spending=accounts.find(a=>a.type==='SPENDING')?.id; const asset=accounts.find(a=>a.type==='PROVIDER_ASSET')?.id;
    if(!spending||!asset)throw new Error('Top-up ledger accounts are missing.');
    const amount=integer(row.credit_baht,'credit'); const ledgerId=crypto.randomUUID();
    await transaction`INSERT INTO ledger_transactions(id,business_reference,event_type,description)
  ...
  private async applyPayoutWebhook(transaction:TransactionSql<Record<string,never>>,row:{id:string;user_id:string;
    principal_baht:string;maximum_debit_baht:string;quoted_fee_satang:string;quoted_tax_satang:string;
    actual_debit_satang:string|null;status:string;final_ledger_transaction_id:string|null},status:string){
    const success=status==='SUCCEEDED'||status==='COMPLETED';
    const failure=['FAILED','REJECTED','CANCELLED','EXPIRED'].includes(status);
    const reversed=status==='REVERSED'; if(!success&&!failure&&!reversed)return;
    if(reversed&&row.status==='REVERSED')return;
    if(reversed&&row.status!=='SUCCEEDED')throw new Error('Payout reversal arrived before a successful payout.');
    if(row.final_ledger_transaction_id&&!reversed)return;
    const [wallet]=(await transaction`SELECT id::text FROM wallets WHERE user_id=${row.user_id}`) as unknown as Array<{id:string}>;
    const accounts=(await transaction`SELECT id::text,type FROM ledger_accounts WHERE
      (wallet_id=${wallet?.id} AND type IN ('EARNINGS','PAYOUT_RESERVED')) OR code='SYSTEM:PAYOUT_CLEARING'`) as unknown as Array<{id:string;type:string}>;
    const earnings=accounts.find(a=>a.type==='EARNINGS')?.id; const reserved=accounts.find(a=>a.type==='PAYOUT_RESERVED')?.id;
    const clearing=accounts.find(a=>a.type==='PAYOUT_CLEARING')?.id; if(!earnings||!reserved||!clearing)throw new Error('Payout ledger accounts are missing.');
    ...
    const exactDebitSatang=integer(row.principal_baht,'principal_baht')*100+quotedFeeSatang+quotedTaxSatang;
    const actualDebitBaht=reversed
      ? Math.ceil(integer(row.actual_debit_satang??String(exactDebitSatang),'actual_debit_satang')/100)
      : success?Math.ceil(exactDebitSatang/100):0;
    ...
    if(success){
      ...
      if(unusedReserve>0)await transaction`INSERT INTO ledger_postings(transaction_id,account_id,amount_baht)
