...
export const canonicalizeProviderPayload = (value: unknown): string => {
  ...
  if (typeof value === 'number') {
    if (!Number.isFinite(value))
      throw new ProviderEventError(
        'PROVIDER_EVENT_INVALID',
        'Provider payload contains an invalid number.'
      );
    return JSON.stringify(value);
  }
  ...
  if (typeof value === 'object') {
    ...
    return `{${Object.keys(object)
      .sort()
      .map((key) => `${JSON.stringify(key)}:${canonicalizeProviderPayload(object[key])}`)
      .join(',')}}`;
  }
  throw new ProviderEventError(
    'PROVIDER_EVENT_INVALID',
    'Provider payload contains an unsupported value.'
  );
};
...
const parseProviderCaptureAmount = (captures: Record<string, unknown>[]): Satang => {
  ...
  if (!Number.isSafeInteger(amount) || amount <= 0 || amount > MAX_WALLET_CAPACITY_SATANG) {
    throw new ProviderEventError(
      'PROVIDER_EVENT_INVALID',
      'Provider capture amount is out of range.'
    );
  }
  return positiveSatang(amount);
};
...
const parseProviderDate = (value: unknown, fallback: Date): Date => {
  ...
  const text = asText(value);
  if (!text)
    throw new ProviderEventError('PROVIDER_EVENT_INVALID', 'Provider event timestamp is invalid.');
...
export const isTopUpProviderReversal = (providerStatus: string, eventType?: string): boolean =>
  eventType === 'refund.succeeded' ||
  ['REVERSED', 'REFUNDED', 'REFUND', 'CHARGEBACK', 'CHARGEBACKED'].includes(
...
export const parseTopUpProviderEvent = (
  ...
  if (rawPayloadBytes === 0 || rawPayloadBytes > maxRawPayloadBytes) {
    throw new ProviderEventError(
      'PROVIDER_EVENT_INVALID',
      'Provider payload is empty or too large.'
    );
  }
  ...
  if (
    !eventType ||
    !supportedProviderEventTypes.includes(eventType as (typeof supportedProviderEventTypes)[number])
  ) {
  ...
  if (!providerStatus)
    throw new ProviderEventError('PROVIDER_EVENT_INVALID', 'Provider status is required.');
  ...
  if (!internalReference && !providerReference) {
    throw new ProviderEventError(
      'PROVIDER_EVENT_INVALID',
      'Provider event has no payment reference.'
    );
  }
  ...
  if (providerCountry && providerCountry !== 'TH') {
    throw new ProviderEventError(
      'PROVIDER_EVENT_INVALID',
      'Provider event country is not supported.'
    );
  }
  ...
  if (providerCurrency && providerCurrency !== 'THB') {
    throw new ProviderEventError(
      'PROVIDER_EVENT_INVALID',
      'Provider event currency is not supported.'
    );
  }
  ...
  if (
    ...
    throw new ProviderEventError(
      'PROVIDER_EVENT_INVALID',
      'Provider event channel is not supported.'
    );
  }
  ...
  const captures = Array.isArray(data.captures)
    ? data.captures.map((capture) => {
        ...
        if (!object)
          throw new ProviderEventError('PROVIDER_EVENT_INVALID', 'Provider capture is invalid.');
        return object;
      })
    : [];
  const capturedAmountSatang = captures.length > 0 ? parseProviderCaptureAmount(captures) : null;
  const providerAmount =
    capturedAmountSatang ??
    data.request_amount ??
    data.amount ??
    data.paid_amount ??
  ...
  const normalizedStatus =
    eventType === 'refund.succeeded' ? 'FAILED' : normalizeTopUpOutcomeStatus(providerStatus);
  const providerAmountSatang =
    providerAmount === undefined || providerAmount === null
      ? null
      : (capturedAmountSatang ?? parseProviderAmount(providerAmount));
  if (normalizedStatus === 'PAID' && providerAmountSatang === null) {
    throw new ProviderEventError(
      'PROVIDER_EVENT_INVALID',
      'A paid Provider event must include an amount.'
    );
  }
  ...
  const resolvedProviderEventId =
    firstText(providerEventId, payload.event_id, payload.id, data.event_id) ??
    (providerEventIdentity
      ? `derived:${eventType}:${providerEventIdentity}:${normalizedStatus}`
      : null);
  if (!resolvedProviderEventId) {
    throw new ProviderEventError(
      'PROVIDER_EVENT_INVALID',
      'Provider event identifier is required.'
    );
  }
