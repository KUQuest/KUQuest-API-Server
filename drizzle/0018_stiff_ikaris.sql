...
CREATE TABLE "wallet_funding_reservations" (
	...
	CONSTRAINT "wallet_funding_reservations_amounts_check" CHECK ("wallet_funding_reservations"."total_reserved_satang" BETWEEN 1 AND 2000000000 AND "wallet_funding_reservations"."remaining_satang" BETWEEN 0 AND "wallet_funding_reservations"."total_reserved_satang"),
...
ALTER TABLE "wallet_ledger_transactions" ADD CONSTRAINT "wallet_ledger_transactions_event_type_check" CHECK ("wallet_ledger_transactions"."event_type" IN ('TOP_UP', 'PAYOUT', 'FUNDING_RESERVE', 'FUNDING_RELEASE', 'FUNDING_SETTLEMENT', 'ADJUSTMENT', 'EARNINGS_CONVERSION'));
--> statement-breakpoint
...
FOR EACH ROW EXECUTE FUNCTION wallet_validate_funding_reservation_integrity_update();--> statement-breakpoint
...
FOR EACH ROW EXECUTE FUNCTION wallet_reject_funding_settlement_mutation();--> statement-breakpoint
...
FOR EACH ROW EXECUTE FUNCTION wallet_reject_funding_reservation_operation_mutation();--> statement-breakpoint
