ALTER TABLE money_policy_revisions
  ADD COLUMN top_up_provider_fee_satang bigint NOT NULL DEFAULT 0,
  ADD COLUMN top_up_provider_tax_bps bigint NOT NULL DEFAULT 0,
  ADD COLUMN payout_provider_fee_satang bigint NOT NULL DEFAULT 0,
  ADD COLUMN payout_provider_tax_bps bigint NOT NULL DEFAULT 0;

ALTER TABLE money_policy_revisions DROP CONSTRAINT money_policy_fee_chk;
...
-- The showcase policy intentionally makes wallet movements easy to inspect:
-- the worker receives the whole funded amount and provider fees default to zero.
...
SELECT 2, minimum_top_up_baht, maximum_top_up_baht,
  minimum_funded_job_baht, maximum_funded_job_baht,
  minimum_earnings_conversion_baht, maximum_earnings_conversion_baht,
  minimum_payout_baht, maximum_payout_baht, 0,
  0, 0, 0, 0, dispute_two_person_threshold_baht,
  quote_lifetime_seconds, review_window_seconds,
...
CREATE INDEX development_actor_sessions_user_idx
  ON development_actor_sessions(user_id);
CREATE INDEX development_actor_sessions_expiry_idx
  ON development_actor_sessions(expires_at);

DO $$
BEGIN
  IF to_regrole('kuquest_app') IS NOT NULL THEN
    GRANT SELECT ON
      jobs, job_status_history, job_applications,
    ...
    GRANT INSERT ON
      ...
      development_test_users, development_actor_sessions
      TO kuquest_app;

    GRANT UPDATE ON
      jobs, job_applications, work_submissions, top_up_quotes, top_ups,
      payout_accounts, payout_quotes, payouts, provider_webhook_events,
      scheduled_tasks
      TO kuquest_app;
