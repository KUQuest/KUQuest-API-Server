...
- **Chaining guards**: guards are Elysia plugins, applied in the order their checks depend on each other — `.use(authGuard).use(adminAuthenticationGuard)`, not the reverse, since `adminAuthenticationGuard` reads `adminSession` off the context it derives. Each guard does exactly one check (`onBeforeHandle` + `status(...)`/`set.status`); don't fold multiple unrelated checks into one guard file — add a new guard plugin and chain it instead.
...
- **Authenticated requests**: use `authenticateAsStudent()` from `tests/support/authenticate-as-student.ts` to get a real Session for a guard-protected endpoint, then send its `cookie` on the request — don't stub `auth.api.getSession` or invent another workaround. Call `cleanupStudentSessions()` (same file) in an `afterAll` with every `userId` it returned. Prefer this over asserting a guarantee at a lower seam (service/controller-only) purely because no Session was reachable; see `docs/adr/0002-profile-ownership-proven-below-http.md` for the seam this fixture retires.
